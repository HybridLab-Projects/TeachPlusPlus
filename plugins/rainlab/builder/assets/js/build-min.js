+(function ($) {
    ...
    DataRegistry.prototype.localizationUpdated = function (plugin, params) {
        ...
        if (params === undefined || !params.suppressLanguageEditorUpdate) {
            $.oc.builder.indexController.entityControllers.localization.languageUpdated(
                plugin
            );
        }
        $.oc.builder.indexController.entityControllers.localization.updateOnScreenStrings(
            plugin
        );
    };
...
+(function ($) {
    ...
    DatabaseTable.prototype.onTableLoaded = function () {
        ...
        var $masterTabPane = this.getMasterTabsActivePane(),
            ...
            $addIdButton = $(
                '<a class="btn oc-icon-clock-o builder-custom-table-button" data-builder-command="databaseTable:cmdAddIdColumn"></a>'
            ),
            $addTimestampsButton = $(
                '<a class="btn oc-icon-clock-o builder-custom-table-button" data-builder-command="databaseTable:cmdAddTimestamps"></a>'
            ),
            $addSoftDeleteButton = $(
                '<a class="btn oc-icon-refresh builder-custom-table-button" data-builder-command="databaseTable:cmdAddSoftDelete"></a>'
            );
    ...
    DatabaseTable.prototype.saveMigrationDone = function (data) {
        ...
        $.oc.builder.dataRegistry.clearCache(
...
+(function ($) {
    ...
    ModelForm.prototype.cmdSaveForm = function (ev) {
        var $target = $(ev.currentTarget),
            ...
            controls = $.oc.builder.formbuilder.domToPropertyJson.convert(
                $rootContainer.get(0)
            );
        if (
            !$.oc.inspector.manager.applyValuesFromContainer(
                $inspectorContainer
            )
        ) {
            return;
...
+(function ($) {
    ...
    ModelList.prototype.addColumn = function ($target, column, type) {
        var tableObj = this.getTableControlObject($target),
            currentData = this.getTableData($target),
            rowData = { field: column, label: column, type: type };
        tableObj.addRecord("bottom", true);
        tableObj.setRowValues(currentData.length - 1, rowData);
        tableObj.addRecord("bottom", false);
        tableObj.deleteRecord();
    ...
    ModelList.prototype.onListLoaded = function () {
        ...
        var $masterTabPane = this.getMasterTabsActivePane(),
            ...
            $button = $(
                '<a class="btn oc-icon-magic builder-custom-table-button" data-builder-command="modelList:cmdAddDatabaseColumns"></a>'
            );
...
+(function ($) {
    ...
    Version.prototype.showHintPopup = function (
        ...
        if (callback) {
            $masterTabPane.one("shown.oc.popup", function (
                ev,
                $element,
                $modal
            ) {
...
+(function ($) {
    ...
    Controller.prototype.cmdCreateController = function (ev) {
        var $form = $(ev.currentTarget),
            ...
            behaviorsSelected = $form.find('input[name="behaviors[]"]:checked')
                .length,
            promise = null;
...
+(function ($) {
    ...
    Builder.prototype.createEntityControllers = function () {
        for (var controller in $.oc.builder.entityControllers) {
            ...
            this.entityControllers[
                controller
            ] = new $.oc.builder.entityControllers[controller](this);
        }
    };
...
+(function ($) {
    ...
    LocalizationInput.prototype.loadDataAndBuild = function () {
        ...
        var result = $.oc.builder.dataRegistry.get(
                this.form,
                this.options.plugin,
                "localization",
                null,
    ...
    LocalizationProcessor.prototype.buildLocalizationEditor = function () {
        ...
        this.localizationInput = new $.oc.builder.localizationInput(
            ...
            $(input),
            {
                ...
                autocompleteOptions: {
                    menu:
                        '<ul class="autocomplete dropdown-menu table-widget-autocomplete localization"></ul>',
                    bodyContainer: true,
                },
            }
        );
