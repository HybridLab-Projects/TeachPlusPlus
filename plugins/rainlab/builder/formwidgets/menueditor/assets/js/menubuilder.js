...
+(function ($) {
    ...
    if ($.oc.builder.menubuilder === undefined) $.oc.builder.menubuilder = {};
    ...
    MenuBulder.prototype.registerHandlers = function () {
        $(document).on(
            "change",
            ".builder-menu-editor li.item",
            this.proxy(this.onItemChange)
        );
        $(document).on(
            "dragged.list.sortable",
            ".builder-menu-editor li.item",
            this.proxy(this.onItemDragged)
        );
        $(document).on(
            "livechange",
            ".builder-menu-editor li.item",
            this.proxy(this.onItemLiveChange)
        );
    };
    ...
    MenuBulder.prototype.getTemplateMarkup = function (
        element,
        templateAttribute
    ) {
    ...
    MenuBulder.prototype.getItemProperties = function (item) {
        return $.parseJSON(
            $(item).find("> input[data-inspector-values]").val()
        );
    };

    MenuBulder.prototype.itemCodeExistsInList = function ($list, code) {
        var valueInputs = $list.find(
            "> li.item > input[data-inspector-values]"
        );
    ...
    MenuBulder.prototype.replacePropertyValue = function (
        $item,
        property,
        value
    ) {
    ...
    MenuBulder.prototype.updateItemVisualProperties = function (item) {
        ...
        $item
            .find("> .item-container > span.title")
            .attr("data-localization-key", properties.label);

        $.oc.builder.dataRegistry.getLocalizationString(
            $item,
            pluginCode,
            properties.label,
            function (label) {
    ...
    MenuBulder.prototype.addMainMenuItem = function (ev) {
        var newItemMarkup = this.getTemplateMarkup(
                ev.currentTarget,
                "data-main-menu-template"
            ),
            $item = $(newItemMarkup),
    ...
    MenuBulder.prototype.getJson = function (form) {
        var mainMenuItems = form.querySelectorAll(
                "ul.builder-main-menu > li.item"
            ),
            result = [];
        ...
        for (var i = 0, lenOuter = mainMenuItems.length; i < lenOuter; i++) {
            ...
            var sideMenuItems = mainMenuItem.querySelectorAll(
                "ul.builder-submenu > li.item"
            );
            for (
                var j = 0, lenInner = sideMenuItems.length;
                j < lenInner;
                j++
            ) {
    ...
    MenuBulder.prototype.deleteMenuItem = function (ev) {
        ...
        for (var i = subitems.length - 1; i >= 0; i--) {
            var $inspectorContainer = this.findInspectorContainer(
                $(subitems[i])
            );
            $.oc.foundation.controlUtils.disposeControls(
                $inspectorContainer.get(0)
