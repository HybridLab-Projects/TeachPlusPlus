...
+(function ($) {
    ...
    TabManager.prototype.registerHandlers = function () {
        ...
        $layoutBody.on(
            "click",
            "li[data-builder-new-tab]",
            this.proxy(this.onNewTabClick)
        );
        $layoutBody.on(
            "click",
            "div[data-builder-tab]",
            this.proxy(this.onTabClick)
        );
        $layoutBody.on(
            "click",
            "div[data-builder-close-tab]",
            this.proxy(this.onTabCloseClick)
        );
        $layoutBody.on(
            "change livechange",
            "ul.tabs > li div.inspector-trigger.tab-control",
            this.proxy(this.onTabChange)
        );
        $layoutBody.on(
            "hiding.oc.inspector",
            "ul.tabs > li div.inspector-trigger.tab-control",
            this.proxy(this.onTabInspectorHiding)
        );
    };
    ...
    TabManager.prototype.getGlobalTabsProperties = function (tabsContainer) {
        var properties = $(tabsContainer)
            .find(
                ".inspector-trigger.tab-control.global [data-inspector-values]"
            )
            .val();
    ...
    TabManager.prototype.tabHasControls = function ($tab) {
        return (
            this.findTabPanel($tab).find(
                "ul[data-control-list] li.control:not(.placeholder)"
            ).length > 0
        );
    };
    ...
    TabManager.prototype.createNewTab = function ($tabControl) {
        var tabTemplate = $("[data-tab-template]", $tabControl).html(),
            ...
            patchedTabTemplate = tabTemplate.replace(/tabtitle/gm, tabName);
    ...
    TabManager.prototype.closeTabInspectors = function ($tab, $tabPanel) {
        if (
            $tab.find(".inspector-open").length === 0 &&
            $tabPanel.find(".inspector-open").length === 0
        ) {
            return;
        }
    ...
    TabManager.prototype.updateTabProperties = function ($tab) {
        ...
        $tab.find("[data-tab-title]").attr(
            "data-localization-key",
            propertiesParsed.title
        );
