...
+(function ($) {
    ...
    FormBuilder.prototype.registerHandlers = function () {
        ...
        $(document).on(
            "change",
            ".builder-control-list > li.control",
            this.proxy(this.onControlChange)
        );
        $(document).on(
            "click",
            ".builder-control-list > li.control div[data-builder-remove-control]",
            this.proxy(this.onRemoveControl)
        );
        $(document).on(
            "click",
            ".builder-control-list > li.placeholder",
            this.proxy(this.onPlaceholderClick)
        );
        $(document).on(
        ...
        $(document).on(
            "livechange",
            ".builder-control-list > li.control",
            this.proxy(this.onControlLiveChange)
        );
        $(document).on(
        ...
        $(document).on(
            ...
            ".builder-control-list > li.control",
            this.proxy(this.onDropdownOptions)
        );
    };
    ...
    FormBuilder.prototype.getControlProperties = function (li) {
        ...
        for (var i = children.length - 1; i >= 0; i--) {
            ...
            if (
                ...
                element.hasAttribute("data-inspector-values")
            ) {
    ...
    FormBuilder.prototype.loadModelFields = function (control, callback) {
        ...
        $.oc.builder.dataRegistry.get(
            ...
            function (response) {
                callback({
                    options: $.oc.builder.indexController.dataToInspectorArray(
                        response
                    ),
                });
            }
    ...
    FormBuilder.prototype.getContainerFieldNames = function (
        control,
        callback
    ) {
        var controlWrapper = this.findRootControlWrapper(control),
            fieldNames = $.oc.builder.formbuilder.domToPropertyJson.getAllControlNames(
                controlWrapper
            ),
            options = [];
    ...
    FormBuilder.prototype.fieldNameExistsInContainer = function (
        container,
        fieldName
    ) {
        var valueInputs = container.querySelectorAll(
            "li.control[data-inspectable] input[data-inspector-values]"
        );
    ...
    FormBuilder.prototype.reflow = function (li, listElement) {
        if (!li && !listElement) {
            throw new Error(
                "Invalid call of the reflow method. Either li or list parameter should be not empty."
            );
        }
        ...
        for (var i = 0, len = items.length; i < len; i++) {
            ...
            if (itemSpan == "auto") {
                ...
                if (prevSpan == "left") {
                    ...
                    if (
                        !$.oc.foundation.element.hasClass(item, "placeholder")
                    ) {
    ...
    FormBuilder.prototype.setControlSpanFromProperties = function (
        li,
        properties
    ) {
    ...
    FormBuilder.prototype.dropTargetIsChildOf = function (target, ev) {
        ...
        while (current) {
            if (
                this.elementIsControl(current) &&
                this.dataTransferContains(ev, this.getControlId(current))
            ) {
                return true;
            }
    ...
    FormBuilder.prototype.dropFromContainerToPlaceholderOrControl = function (
        ev,
        targetControl
    ) {
        ...
        var controlId = ev.dataTransfer.getData("builder/control/id"),
            originalControl = document.body.querySelector(
                'li[data-builder-control-id="' + controlId + '"]'
            );
        ...
        var isSameList =
                originalControl.parentNode === targetElement.parentNode,
        ...
        if (
            !$.oc.foundation.element.hasClass(originalControl, "inspector-open")
        ) {
    ...
    FormBuilder.prototype.elementContainsPoint = function (point, element) {
        ...
        return (
            point.x >= elementPosition.left &&
            point.x <= elementRight &&
            point.y >= elementPosition.top &&
            point.y <= elementBottom
        );
    };

    FormBuilder.prototype.stopHighlightingTargets = function (
        target,
        excludeTarget
    ) {
    ...
    FormBuilder.prototype.updateControlBody = function (controlId) {
        var control = document.body.querySelector(
            'li[data-builder-control-id="' + controlId + '"]'
        );
        if (!control) {
            return;
        ...
        var rootWrapper = this.findRootControlWrapper(control),
            controls = rootWrapper.querySelectorAll(
                "li.control.updating-control"
            );
        ...
        $(control)
            .request("onModelFormRenderControlBody", {
                data: data,
            })
    ...
    FormBuilder.prototype.controlBodyMarkupLoaded = function (responseData) {
        var li = document.body.querySelector(
            'li[data-builder-control-id="' + responseData.controlId + '"]'
        );
        if (!li) {
            return;
        }
    ...
    FormBuilder.prototype.generateFieldName = function (
        controlType,
        placeholder
    ) {
    ...
    FormBuilder.prototype.addControlToPlaceholder = function (
        placeholder,
        controlType,
        controlName,
        noNewPlaceholder
        ...
        if (!noNewPlaceholder) {
            ...
            placeholder.insertAdjacentHTML(
                "afterend",
                newPlaceholder.get(0).outerHTML
            );
        }
        ...
        return $(placeholder)
            .request("onModelFormRenderControlWrapper", {
                data: data,
            })
    ...
    FormBuilder.prototype.controlWrapperMarkupLoaded = function (responseData) {
        var placeholder = document.body.querySelector(
            'li[data-builder-control-id="' + responseData.controlId + '"]'
        );
        if (!placeholder) {
            return;
        ...
        placeholder.setAttribute(
            ...
            responseData.controlTitle
        );
        placeholder.setAttribute(
            "data-inspector-description",
    ...
    FormBuilder.prototype.displayControlPaletteForPlaceholder = function (
        element
    ) {
        $.oc.builder.formbuilder.controlPalette.loadControlPalette(
            element,
            this.getControlId(element)
        );
    };

    FormBuilder.prototype.addControlFromControlPalette = function (
        placeholderId,
        controlType,
        controlName
    ) {
        var placeholder = document.body.querySelector(
            'li[data-builder-control-id="' + placeholderId + '"]'
        );
        if (!placeholder) {
            return;
        }

        return this.addControlToPlaceholder(
            placeholder,
            controlType,
            controlName
        );
    ...
    FormBuilder.prototype.findControlContainer = function (element) {
        ...
        while (current) {
            if (
                ...
                current.hasAttribute("data-control-container")
            ) {
                return current;
            }
    ...
    FormBuilder.prototype.findPlaceholder = function (controlList) {
        ...
        for (var i = children.length - 1; i >= 0; i--) {
            ...
            if (
                element.tagName === "LI" &&
                $.oc.foundation.element.hasClass(element, "placeholder")
            ) {
                return element;
            }
        }
    ...
    FormBuilder.prototype.elementIsControl = function (element) {
        return (
            element.tagName === "LI" &&
            element.hasAttribute("data-control-type") &&
            $.oc.foundation.element.hasClass(element, "control")
        );
    };
    ...
    FormBuilder.prototype.onDragOver = function (ev) {
        ...
        if (
            (this.targetIsPlaceholder(ev) || elementIsControl) &&
            sourceIsContainer
        ) {
            ...
            if (
                sourceIsContainer &&
                elementIsControl &&
                this.dropTargetIsChildOf(targetLi, ev)
            ) {
                return false;
            }
    ...
    FormBuilder.prototype.onDragDrop = function (ev) {
        ...
        if (
            (elementIsControl || this.targetIsPlaceholder(ev)) &&
            sourceIsContainer
        ) {
            this.stopHighlightingTargets(targetLi);
    ...
    FormBuilder.prototype.onInspectorShowing = function (ev) {
        if (
            $(ev.target).find("input[data-non-inspectable-control]").length > 0
        ) {
            ev.preventDefault();
            return false;
        }
