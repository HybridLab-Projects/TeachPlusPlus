...
+(function ($) {
    ...
    ControlPalette.prototype.loadControlPalette = function (
        element,
        controlId
    ) {
        if (this.controlPaletteMarkup === null) {
            ...
            $(element)
                .request("onModelFormLoadControlPalette", {
                    data: data,
                })
                .done(this.proxy(this.controlPaletteMarkupLoaded))
                .always(function () {
    ...
    ControlPalette.prototype.controlPaletteMarkupLoaded = function (
        responseData
    ) {
    ...
    ControlPalette.prototype.showControlPalette = function (
        controlId,
        initControls
    ) {
    ...
    ControlPalette.prototype.assignControlIdToTemplate = function (
        template,
        controlId
    ) {
    ...
    ControlPalette.prototype.markPlaceholderPaletteNotOpen = function (
        control
    ) {
    ...
    ControlPalette.prototype.addControl = function (ev) {
        var $target = $(ev.currentTarget),
            controlId = $target
                .closest("[data-control-palette-controlid]")
                .attr("data-control-palette-controlid");
        ...
        var promise = $.oc.builder.formbuilder.controller.addControlFromControlPalette(
            controlId,
    ...
    ControlPalette.prototype.showControlPalletteInPopup = function (
        controlId,
        initControls
    ) {
        var control = this.getControlById(controlId);
        ...
        $control.ocPopover({
            content: this.assignControlIdToTemplate(
                this.getPopoverMarkup(),
                controlId
            ),
    ...
    ControlPalette.prototype.getPopoverMarkup = function () {
        ...
        var outerMarkup = $(
            "script[data-template=control-palette-popover]"
        ).html();

        this.popoverMarkup = outerMarkup.replace(
            "%s",
            this.controlPaletteMarkup
        );
    ...
    ControlPalette.prototype.dockToContainer = function (ev) {
        var $popoverBody = $(ev.target).closest(".control-popover"),
            $controlIdContainer = $popoverBody.find(
                "[data-control-palette-controlid]"
            ),
            controlId = $controlIdContainer.attr(
                "data-control-palette-controlid"
            ),
    ...
    ControlPalette.prototype.showControlPalletteInContainer = function (
        controlId,
        initControls
    ) {
        var control = this.getControlById(controlId);
        ...
        if (
            !inspectorManager.applyValuesFromContainer($container) ||
            !inspectorManager.containerHidingAllowed($container)
        ) {
            return;
        }
        ...
        var template = this.assignControlIdToTemplate(
            this.getContainerMarkup(),
            controlId
        );
        $container.append(template);

        $container
            .find("[data-control-palette-controlid]")
            .one(
                "dispose-control",
                this.proxy(this.onRemovePaletteFromContainer)
            );
    ...
    ControlPalette.prototype.removePaletteFromContainer = function (
        $container
    ) {
    ...
    ControlPalette.prototype.getContainerMarkup = function () {
        ...
        var outerMarkup = $(
            "script[data-template=control-palette-container]"
        ).html();

        this.containerMarkup = outerMarkup.replace(
            "%s",
            this.controlPaletteMarkup
        );
    ...
    ControlPalette.prototype.closeInContainer = function (ev) {
        this.removePaletteFromContainer(
            $(ev.target).closest("[data-control-palette-controlid]")
        );
    };

    ControlPalette.prototype.undockFromContainer = function (ev) {
        var $container = $(ev.target).closest(
                "[data-control-palette-controlid]"
            ),
