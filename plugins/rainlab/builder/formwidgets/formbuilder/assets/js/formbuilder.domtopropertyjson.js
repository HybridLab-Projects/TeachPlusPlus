...
+(function ($) {
    ...
    if ($.oc.builder.formbuilder === undefined) $.oc.builder.formbuilder = {};
    ...
    function getControlPropertyValues(item) {
        for (var i = 0, len = item.children.length; i < len; i++) {
            ...
            if (
                child.tagName === "INPUT" &&
                child.hasAttribute("data-inspector-values")
            ) {
    ...
    function preProcessSpecialProperties(properties) {
        ...
        if (
            String(properties["oc.comment"]).length > 0 &&
    ...
    function parseControlControlContainer(control) {
        ...
        for (var i = 0, len = children.length; i < len; i++) {
            ...
            if (
                child.tagName !== "DIV" ||
                !child.hasAttribute("data-control-container") ||
                !child.hasAttribute("data-container-name")
            ) {
                continue;
            }

            var containerName = String(
                    child.getAttribute("data-container-name")
                ),
    ...
    function listToJson(list, injectProperties) {
        ...
        for (var i = 0, len = listItems.length; i < len; i++) {
            ...
            if (values === null) {
                throw new Error(
                    "Property values are not found for a control list item."
                );
            }

            if (values["oc.fieldName"] === undefined) {
                throw new Error(
                    "Field name property is not found for a control."
                );
            }
    ...
    function getControlListContainerLists(controlListContainer) {
        var controlLists = controlListContainer.querySelectorAll(
                "[data-control-list]"
            ),
            result = [],
            listName = null;
        ...
        for (var i = 0, len = controlLists.length; i < len; i++) {
            var controlList = controlLists[i],
                currentListName = String(
                    controlList.getAttribute("data-control-list")
                ),
    ...
    function getControlListParentContainer(controlList) {
        ...
        while (parent) {
            ...
            if (
                parent.hasAttribute("data-control-list-container") ||
                parent.hasAttribute("data-control-container")
            ) {
                return parent;
            }
    ...
    function mergeListContainerControlsToResult(result, container) {
        var listContainerType = String(
            container.getAttribute("data-control-list-container-type")
        );
        ...
        if (listContainerType !== "tabs") {
            ...
            //
            throw new Error(
                "Unknown control list container type: " + listContainerType
            );
        }

        var controlLists = getControlListContainerLists(container),
            globalTabsProperties = $.oc.builder.formbuilder.tabManager.getGlobalTabsProperties(
                container
            );

        for (var i = 0, len = controlLists.length; i < len; i++) {
            var controlList = controlLists[i],
                tabTitle = $.oc.builder.formbuilder.tabManager.getElementTabTitle(
                    controlList
                ),
                injectProperties = {
                    tab: tabTitle,
            ...
            if (
                ...
                result[listName] = $.extend(
                    result[listName],
                    globalTabsProperties
                );
                mergeControlListControlsToResult(
                    result,
                    listControls,
                    listName
                );
    ...
    function containerToJson(container) {
        ...
        for (var i = 0, len = containerElements.length; i < len; i++) {
            var currentElement = containerElements[i],
                isControlListContainer = currentElement.hasAttribute(
                    "data-control-list-container"
                );

            if (currentElement.tagName !== "UL" && !isControlListContainer) {
                throw new Error(
                    "Control container can contain only UL elements and control list containers."
                );
            }
            ...
            if (isControlListContainer) {
                ...
                if (!currentElement.hasAttribute("data-control-list")) {
                    throw new Error(
                        "Control container can contain only UL elements with data-control-list attribute or control list containers."
                    );
                }
            }
    ...
    DomToJson.getAllControlNames = function (rootContainer) {
        var controls = rootContainer.querySelectorAll(
                "ul[data-control-list] > li.control"
            ),
            result = [];
