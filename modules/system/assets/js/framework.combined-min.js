...
+(function ($) {
    ...
    var Request = function (element, handler, options) {
        ...
        var loading = options.loading !== undefined ? options.loading : null,
            url =
                options.url !== undefined ? options.url : window.location.href,
            isRedirect =
                options.redirect !== undefined && options.redirect.length,
        ...
        if ($el.is(":input") && !$form.length) {
            ...
            if (
                inputName !== undefined &&
                options.data[inputName] === undefined
            ) {
        ...
        var requestOptions = {
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                if (useFlash && data["X_OCTOBER_FLASH_MESSAGES"]) {
                    $.each(data["X_OCTOBER_FLASH_MESSAGES"], function (
                        type,
                        message
                    ) {
            ...
            handleValidationMessage: function (message, fields) {
                $triggerEl.trigger("ajaxValidation", [
                    context,
                    message,
                    fields,
                ]);
                var isFirstInvalidField = true;
                $.each(fields, function focusErrorField(
                    fieldName,
                    fieldMessages
                ) {
...
+(function (window) {
    ...
    function getBody(str, pos) {
        ...
        throw new Error(
            "Broken JSON body near " +
                str.substr(pos - 5 >= 0 ? pos - 5 : 0, 50)
        );
    }
...
+(function ($) {
    ...
    $(document).on("ajaxSetup", "[data-request][data-request-flash]", function (
        event,
        context
    ) {
