...
+(function ($) {
    ...
    ExceptionBeautifier.EDITORS = {
        subl: {
            scheme: "subl://open?url=file://%file&line=%line",
            name: "Sublime (subl://)",
        },
        txmt: {
            scheme: "txmt://open/?url=file://%file&line=%line",
            name: "TextMate (txmt://)",
        },
        mvim: {
            scheme: "mvim://open/?url=file://%file&line=%line",
            name: "MacVim (mvim://)",
        },
        phpstorm: {
            scheme: "phpstorm://open?file=%file&line=%line",
            name: "PhpStorm (phpstorm://)",
        },
        editor: {
            scheme: "editor://open/?file=%file&line=%line",
            name: "Custom (editor://)",
        },
    };
    ...
    ExceptionBeautifier.prototype.initEditorPopup = function () {
        if (!ExceptionBeautifier.LINKER_POPUP_CONTENT) {
            var title = $.oc.lang.get("eventlog.editor.title"),
                ...
                rememberChoice = $.oc.lang.get(
                    "eventlog.editor.remember_choice"
                ),
...
<div>                                                                                                                  \
    ...
    <div class="modal-body">                                                                                           \
        ...
        <div class="checkbox custom-checkbox">                                                                         \
            ...
            <label for="editor-remember-choice">' +
                        rememberChoice +
                        '</label>                                         \
        </div>                                                                                                         \
    </div>                                                                                                             \
    <div class="modal-footer">                                                                                         \
        <button type="button" class="btn btn-primary" data-action="submit" data-dismiss="modal">' +
                        open +
                        '</button>  \
                ...
                if (ExceptionBeautifier.EDITORS.hasOwnProperty(key)) {
                    select.append(
                        '<option value="' +
                            ...
                            '">' +
                            $.oc.escapeHtmlString(
                                ExceptionBeautifier.EDITORS[key].name
                            ) +
                            "</option>"
                    );
                }
    ...
    ExceptionBeautifier.prototype.openWithEditor = function (link) {
        ...
        if ((matches = link.match(ExceptionBeautifier.REGEX.editor))) {
            if (
                ...
                window.sessionStorage["oc-exception-beautifier-editor"]
            ) {
                ...
                $.popup({ content: ExceptionBeautifier.LINKER_POPUP_CONTENT })
                    .on("shown.oc.popup", function (event, source, popup) {
                        ...
                        $('[data-action="submit"]', popup).on(
                            "click",
                            function () {
                                if (
                                    $("#editor-remember-choice").prop(
                                        "checked"
                                    ) &&
                                    window.sessionStorage
                                ) {
                                    window.sessionStorage[
                                        "oc-exception-beautifier-editor"
                                    ] = select.val();
                                }
    ...
    ExceptionBeautifier.prototype.formatFilePath = function (path, line) {
        ...
        return (
            '{exception-beautifier-file#a href="javascript:" data-href="idelink://' +
            encodeURIComponent(self.rewritePath(path)) +
            "&" +
            line +
            '"}' +
            path +
            "{/exception-beautifier-file#a}"
        );
    };
