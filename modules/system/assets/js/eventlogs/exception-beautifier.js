...
+(function ($) {
    ...
    ExceptionBeautifier.prototype.init = function () {
        ...
        self.$el.addClass("plugin-exception-beautifier").empty().append(markup);
    };
    ...
    ExceptionBeautifier.prototype.parseSource = function (raw) {
        ...
        if (source.indexOf("Stack trace:") < 0) {
            source =
                "{exception-beautifier-message-container}{exception-beautifier-message}" +
                self.formatMessage(source) +
                "{/exception-beautifier-message}{/exception-beautifier-message-container}";
        } else {
            ...
            source =
                ...
                "{exception-beautifier-message}" +
                self.formatMessage(markup.message) +
                "{/exception-beautifier-message}" +
                "{/exception-beautifier-message-container}" +
            ...
            markup.lines.forEach(function (line) {
                source +=
                    "{exception-beautifier-stacktrace-line}" +
                    self.formatStackTraceLine(line) +
                    "{/exception-beautifier-stacktrace-line}";
            });
        ...
        markup = $(
            self.buildMarkup(
                "{exception-beautifier-container}" +
                    source +
                    "{/exception-beautifier-container}"
            )
        );
    ...
    ExceptionBeautifier.prototype.parseLine = function (str) {
        ...
        if (
            (matches = str.match(ExceptionBeautifier.REGEX.phpline)) ||
            (matches = str.match(ExceptionBeautifier.REGEX.artisan))
        ) {
            line.type = "phpline";
            ...
            (matches = str.match(ExceptionBeautifier.REGEX.internalLine))
        ) {
            line.type = "internal";
            line.number = $.trim(matches[1]);
            ...
            (matches = str.match(ExceptionBeautifier.REGEX.defaultLine))
        ) {
            line.type = "default";
            line.number = $.trim(matches[1]);
    ...
    ExceptionBeautifier.prototype.formatStackTraceLine = function (line) {
        ...
        switch (line.type) {
            case "phpline":
                return (
                    "{exception-beautifier-stacktrace-line-number}" +
                    line.number +
                    "{/exception-beautifier-stacktrace-line-number}" +
                    self.formatFilePath(line.file, line.lineNumber) +
                    "{exception-beautifier-line-number}(" +
                    line.lineNumber +
                    "):{/exception-beautifier-line-number} " +
                    "{exception-beautifier-stacktrace-line-function}" +
                    line.function +
                    "{/exception-beautifier-stacktrace-line-function}"
                );
            ...
            case "internal":
                return (
                    ...
                    line.number +
                    "{/exception-beautifier-stacktrace-line-number}" +
                    "{exception-beautifier-stacktrace-line-internal}" +
                    line.internal +
                    "{/exception-beautifier-stacktrace-line-internal}" +
                    "{exception-beautifier-stacktrace-line-function}" +
                    line.function +
    ...
    ExceptionBeautifier.prototype.formatLineCode = function (str) {
        ...
        if (str.match(/^\s*(call_user_func|spl_autoload_call)/)) {
            str = str.replace(
                /^\s*(?:call_user_func|spl_autoload_call)([^(]*)\((.*)\)/,
                function (str, suffix, parameters) {
                    return (
                        "{exception-beautifier-system-function}call_user_func" +
                        suffix +
                        "({/exception-beautifier-system-function}" +
            ...
            str = str.replace(ExceptionBeautifier.REGEX.closureCall, function (
                str,
                parameters
            ) {
                return (
                    "{exception-beautifier-function}{closure}({/exception-beautifier-function}" +
                    self.formatFunctionParameters(parameters) +
                    "{exception-beautifier-function}){/exception-beautifier-function}"
                );
            });
        } else if (str.match(ExceptionBeautifier.REGEX.functionCall)) {
            str = str.replace(ExceptionBeautifier.REGEX.functionCall, function (
                str,
                functionName,
                parameters
            ) {
                return (
                    "{exception-beautifier-function}→" +
                    functionName +
                    "({/exception-beautifier-function}" +
                    self.formatFunctionParameters(parameters) +
                    "{exception-beautifier-function}){/exception-beautifier-function}"
            ...
            str = str.replace(ExceptionBeautifier.REGEX.staticCall, function (
                str,
                functionName,
                parameters
            ) {
        ...
        str = str.replace(ExceptionBeautifier.REGEX.filePath, function (
            str,
            path,
            line,
            lineNumber,
        ...
        str = str.replace(ExceptionBeautifier.REGEX.className, function (
            str,
            name
        ) {
            return (
                "{exception-beautifier-class}" +
                name +
                "{/exception-beautifier-class}"
            );
        });
    ...
    ExceptionBeautifier.prototype.formatFunctionParameters = function (
        parameters
    ) {
        return parameters
            .replace(
                /^([0-9]+)|([^a-z\\])([0-9]+)$|^([0-9]+)([^a-z\\])|([^a-z\\])([0-9]+)([^a-z\\])/g,
                "$2$6{exception-beautifier-number}$1$3$4$7{/exception-beautifier-number}$5$8"
            )
            .replace(
                /^Array$|([^a-z\\])Array$|^Array([^a-z\\])|([^a-z\\])Array([^a-z\\])/g,
                "$1$3{exception-beautifier-code}Array{/exception-beautifier-code}$2$4"
            )
            .replace(
                /^Closure$|(\()Closure(\))/g,
                "$1{exception-beautifier-code}Closure{/exception-beautifier-code}$2"
            )
            .replace(
                /Object\(([^)]+)\)/g,
                "{exception-beautifier-code}Object({/exception-beautifier-code}$1{exception-beautifier-code}){/exception-beautifier-code}"
            )
            .replace(
                /"((?:\\.|[^"])*)"/g,
                '{exception-beautifier-string}"$1"{/exception-beautifier-string}'
            )
            .replace(
                /'((?:\\.|[^'])*)'/g,
                "{exception-beautifier-string}'$1'{/exception-beautifier-string}"
        ...
        if (start >= 0) {
            ...
            while (start >= 0) {
                ...
                if (start > end || start < 0) {
                    markup += self.buildMarkup(
                        str.substring(end, start < 0 ? undefined : start)
                    );
                }
            }
        } else {
    ...
    ExceptionBeautifier.prototype.finalizeMarkup = function (markup, source) {
        var stacktrace, messageContainer, tabs, iframe;

        markup.find(".beautifier-file").each(function () {
            $(this)
                .find(".beautifier-class")
                .each(function () {
                    var $el = $(this);
        ...
        markup
            .find(".beautifier-file+.beautifier-line-number")
            .each(function () {
                var $el = $(this);
                $el.appendTo($el.prev());
            });
        ...
        if (!!stacktrace.length) {
            $(
                '<a class="beautifier-toggle-stacktrace" href="javascript:;"><span>' +
                    $.oc.lang.get("eventlog.show_stacktrace") +
                    "</span></a>"
            )
        ...
        tabs = $(
            '<div class="control-tabs content-tabs tabs-inset">' +
                '<ul class="nav nav-tabs">' +
                '<li class="active"><a href="#beautifier-tab-formatted">' +
                $.oc.lang.get("eventlog.tabs.formatted") +
                "</a></li>" +
                '<li><a href="#beautifier-tab-raw">' +
                $.oc.lang.get("eventlog.tabs.raw") +
                "</a></li>" +
        ...
        if (source.indexOf("Message-ID:") > 0) {
            ...
            iframe = $(
                '<iframe id="#beautifier-tab-formatted-iframe" style="width: 100%; height: 500px; padding: 0" frameborder="0"></iframe>'
            );
        }
        ...
        tabs.find("#beautifier-tab-raw").append(
            '<div class="beautifier-raw-content">' +
                source
                    .trim()
                    .replace(/\r\n|\r|\n/g, "<br>")
                    .replace(/ {2}/g, "&nbsp;&nbsp;") +
    ...
    $.fn.exceptionBeautifier = function (option) {
        ...
        this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.exceptionBeautifier",
                    (data = new ExceptionBeautifier(this, options))
                );
