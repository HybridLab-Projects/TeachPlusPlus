...
+(function ($) {
    ...
    ChangeMonitor.prototype.init = function () {
        ...
        this.$el.on(
            "keyup input paste",
            "input:not(.ace_search_field), textarea:not(.ace_text-input)",
            this.proxy(this.onInputChange)
        );
        $(
            "input:not([type=hidden]):not(.ace_search_field), textarea:not(.ace_text-input)",
            this.$el
        ).each(function () {
    ...
    ChangeMonitor.prototype.onBeforeUnload = function () {
        if (
            $.contains(document.documentElement, this.$el.get(0)) &&
    ...
    $.fn.changeMonitor = function (option) {
        return this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.changeMonitor",
                    (data = new ChangeMonitor(this, options))
                );
        });
    };
