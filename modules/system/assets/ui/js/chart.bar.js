...
+(function ($) {
    ...
    var BarChart = function (element, options) {
        ...
        var $el = (this.$el = $(element)),
            ...
            chartHeight =
                this.options.height !== undefined ? this.options.height : size,
            chartWidth = isFullWidth ? this.$el.width() : size,
            barWidth =
                (chartWidth - (values.values.length - 1) * this.options.gap) /
                values.values.length;

        var $canvas = $("<div/>")
            .addClass("canvas")
            .height(chartHeight)
            .width(isFullWidth ? "100%" : chartWidth);
        $el.prepend($canvas);
        $el.toggleClass("full-width", isFullWidth);

        Raphael(
            $canvas.get(0),
            isFullWidth ? "100%" : chartWidth,
            chartHeight,
            function () {
                self.paper = this;
                self.bars = this.set();
                ...
                $.each(values.values, function (index, valueInfo) {
                    var color =
                            valueInfo.color !== undefined
                                ? valueInfo.color
                                : $.oc.chartUtils.getColor(index),
                        path = self.paper
                            .path()
                            .attr({ "stroke-width": 0 })
                            .attr({ bar: [start, 0] })
                ...
                $.each(values.values, function (index, valueInfo) {
                    var height =
                        values.max && valueInfo.value
                            ? (chartHeight / values.max) * valueInfo.value
                            : 0;

                    self.bars[index].animate(
                        { bar: [start, height] },
                        1000,
                        "bounce"
                    );
                ...
                if (isFullWidth) {
                    $(window).on("resize", function () {
                        ...
                        barWidth =
                            (chartWidth -
                                (values.values.length - 1) * self.options.gap) /
                            values.values.length;
