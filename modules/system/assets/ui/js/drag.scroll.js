...
+(function ($) {
    ...
    var DragScroll = function (element, options) {
        ...
        this.el = $el;
        this.scrollClassContainer = this.options.scrollClassContainer
         ...
         * Bind events
         */
        var $scrollSelect = this.options.scrollSelector
            ? $(this.options.scrollSelector, $el)
            : $el;
        ...
        if (this.options.useDrag) {
            $el.on("mousedown.dragScroll", this.options.dragSelector, function (
                event
            ) {
        ...
        if (Modernizr.touchevents) {
            $el.on(
                "touchstart.dragScroll",
                this.options.dragSelector,
                function (event) {
        ...
        function startDrag(event) {
            ...
            $(window).on("mouseup.dragScroll", function (mouseUpEvent) {
                var isClick =
                    event.pageX == mouseUpEvent.pageX &&
                    event.pageY == mouseUpEvent.pageY;
                stopDrag(isClick);
                return false;
            });
        }
        ...
        function scrollWheel(offset) {
            ...
            if (scrolled) {
                if (
                    self.wheelUpdateTimer !== undefined &&
    ...
    DragScroll.prototype.fixScrollClasses = function () {
        ...
        this.scrollClassContainer.toggleClass(
    ...
    DragScroll.prototype.isEnd = function () {
        if (!this.options.vertical) {
            return (
                this.el[0].scrollWidth -
                    (this.el.scrollLeft() + this.el.width()) <=
                0
            );
        } else {
            return (
    ...
    DragScroll.prototype.isActiveAfter = function () {
        ...
        if (!this.options.vertical) {
            return (
                activeElement.get(0).offsetLeft >
                this.el.scrollLeft() + this.el.width()
            );
        } else {
            return (
                activeElement.get(0).offsetTop >
                this.el.scrollTop() + this.el.height()
            );
        }
    };
    ...
    DragScroll.prototype.isActiveBefore = function () {
        ...
        if (!this.options.vertical) {
            return (
                activeElement.get(0).offsetLeft + activeElement.width() <
                this.el.scrollLeft()
            );
        } else {
            return (
                activeElement.get(0).offsetTop + activeElement.height() <
                this.el.scrollTop()
            );
        }
    };
    ...
    $.fn.dragScroll = function (option) {
        ...
        return this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.dragScroll",
                    (data = new DragScroll(this, options))
                );
