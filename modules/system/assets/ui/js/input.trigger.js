...
+(function ($) {
    ...
    var TriggerOn = function (element, options) {
        ...
        if (
            ...
            $(document).on(
                ...
                $.proxy(this.onConditionChanged, this)
            );
        }
    ...
    TriggerOn.prototype.onConditionChanged = function () {
        if (this.triggerCondition == "checked") {
            this.updateTarget(
                !!$(this.options.trigger + ":checked", this.triggerParent)
                    .length
            );
        } else if (this.triggerCondition == "unchecked") {
            this.updateTarget(
                !$(this.options.trigger + ":checked", this.triggerParent).length
            );
        } else if (this.triggerCondition == "value") {
            var trigger,
                triggered = false;

            trigger = $(this.options.trigger, this.triggerParent).not(
                "input[type=checkbox], input[type=radio], input[type=button], input[type=submit]"
            );

            if (!trigger.length) {
                trigger = $(this.options.trigger, this.triggerParent).not(
                    ":not(input[type=checkbox]:checked, input[type=radio]:checked)"
                );
            }
            ...
            trigger.each(function () {
                ...
                $.each(
                    $.isArray(triggerValue) ? triggerValue : [triggerValue],
                    function (key, val) {
                        triggered =
                            $.inArray(val, self.triggerConditionValue) != -1;
                        return !triggered;
                    }
                );
    ...
    $.fn.triggerOn = function (option) {
        return this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.triggerOn",
                    (data = new TriggerOn(this, options))
                );
        });
    };
