...
+(function ($) {
    ...
    DropdownEditor.prototype.formatSelectOption = function (state) {
        ...
        if (iconClass) {
            return (
                '<i class="select-icon ' + iconClass + '"></i> ' + state.text
            );
        }
    ...
    DropdownEditor.prototype.initCustomSelect = function () {
        ...
        if (!Modernizr.touchevents) {
            ...
            this.indicatorContainer.addClass(
    ...
    DropdownEditor.prototype.onSelectionChange = function () {
        ...
        this.inspector.setPropertyValue(
            ...
            this.normalizeValue(select.value),
            this.initialization
        );
    };

    DropdownEditor.prototype.onInspectorPropertyChanged = function (
        property,
        value
    ) {
        if (
    ...
    DropdownEditor.prototype.loadStaticOptions = function (select) {
        var value = this.inspector.getPropertyValue(
            this.propertyDefinition.property
        );
    ...
    DropdownEditor.prototype.loadDynamicOptions = function (initialization) {
        var currentValue = this.inspector.getPropertyValue(
                this.propertyDefinition.property
            ),
    ...
    DropdownEditor.prototype.optionsRequestDone = function (
        data,
        currentValue,
        initialization
    ) {
        ...
        if (this.hasOptionValue(select, currentValue)) {
            ...
            select.selectedIndex =
                this.propertyDefinition.placeholder === undefined ? 0 : -1;
        }
