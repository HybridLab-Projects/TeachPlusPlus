...
+(function ($) {
    ...
    DataInteraction.prototype.normalizePropertyCode = function (
        code,
        configuration
    ) {
    ...
    DataInteraction.prototype.loadValues = function (configuration) {
        ...
        for (var i = 0, len = attributes.length; i < len; i++) {
            ...
            if ((matches = attribute.name.match(/^data-property-(.*)$/))) {
                ...
                var normalizedPropertyName = normalizePropertyCode(
                    matches[1],
                    configuration
                );
    ...
    DataInteraction.prototype.loadConfiguration = function (onComplete) {
        ...
        if (configurationField) {
            result.configuration = this.parseConfiguration(
    ...
    DataInteraction.prototype.configurartionRequestDone = function (
        data,
        onComplete,
        result
    ) {
