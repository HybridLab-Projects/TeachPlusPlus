...
+(function ($) {
    ...
    FilterWidget.prototype.init = function () {
        ...
        this.$el.on(
            "change",
            '.filter-scope input[type="checkbox"]',
            function () {
        ...
        this.$el.on("show.oc.popover", "a.filter-scope", function (event) {
            ...
            $(event.relatedTarget).on(
                "click",
                "#controlFilterPopover .filter-items > ul > li",
                function () {
            ...
            $(event.relatedTarget).on(
                "click",
                "#controlFilterPopover .filter-active-items > ul > li",
                function () {
            ...
            $(event.relatedTarget).on(
                "ajaxDone",
                "#controlFilterPopover input.filter-search-input",
            ...
            $(event.relatedTarget).on(
                "click",
                '#controlFilterPopover [data-filter-action="apply"]',
                function (e) {
                    e.preventDefault();
                    self.filterScope();
                }
            ...
            $(event.relatedTarget).on(
                "click",
                '#controlFilterPopover [data-filter-action="clear"]',
                function (e) {
                    e.preventDefault();
    ...
    FilterWidget.prototype.bindDependants = function () {
        ...
        $.each(scopeMap, function (scopeName, toRefresh) {
            scopeElements
                .filter('[data-scope-name="' + scopeName + '"]')
                .on(
                    "change.oc.filterScope",
                    $.proxy(
                        self.onRefreshDependants,
                        self,
                        scopeName,
                        toRefresh
                    )
     ...
     */
    FilterWidget.prototype.onRefreshDependants = function (
        scopeName,
        toRefresh
    ) {
        var self = this,
        ...
        this.dependantUpdateTimers[scopeName] = window.setTimeout(function () {
            $.each(toRefresh.scopes, function (index, dependantScope) {
                ...
                var $scope = self.$el.find(
                    '[data-scope-name="' + dependantScope + '"]'
                );
                ...
                self.$el.request(self.options.optionsHandler, {
                    ...
                    success: function (data) {
                        ...
                        self.updateScopeSetting(
                            $scope,
                            data.options.active.length
                        );
    ...
    FilterWidget.prototype.selectItem = function ($item, isDeselect) {
        var $otherContainer = isDeselect
            ? $item
                  ...
                  .find(".filter-items:first > ul")
            : $item
                  .closest(".control-filter-popover")
                  .find(".filter-active-items:first > ul");
        ...
        else this.scopeValues[this.activeScopeName].available = filtered;

        if (item) toItems.push(item);
    ...
    FilterWidget.prototype.displayPopover = function ($scope) {
        ...
        data = $.extend({}, data, {
            apply_button_text: this.getLang(
                "filter.scopes.apply_button_text",
                "Apply"
            ),
            clear_button_text: this.getLang(
                "filter.scopes.clear_button_text",
                "Clear"
            ),
        });
    ...
    FilterWidget.prototype.fillOptions = function (scopeName, data) {
        if (this.scopeValues[scopeName]) return;
    ...
    FilterWidget.prototype.filterAvailable = function (scopeName, available) {
        if (this.activeScopeName != scopeName) return;
    ...
    FilterWidget.prototype.addItemsToListElement = function ($ul, items) {
        $.each(items, function (key, obj) {
            var item = $("<li />")
                .data({ "item-id": obj.id })
                .append(
                    $("<a />").prop({ href: "javascript:;" }).text(obj.name)
                );
    ...
    FilterWidget.prototype.pushOptions = function (scopeName) {
        if (!this.isActiveScopeDirty || !this.options.updateHandler) return;
        ...
        this.$el
            .request(this.options.updateHandler, {
                data: data,
            })
            .always(function () {
    ...
    $.fn.filterWidget = function (option) {
        ...
        this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.filterwidget",
                    (data = new FilterWidget(this, options))
                );
