...
+(function ($) {
    ...
    FilterWidget.prototype.initFilterNumber = function () {
        ...
        this.$el.on("show.oc.popover", "a.filter-scope-number", function (
            ...
            $(event.relatedTarget).on(
                "click",
                '#controlFilterPopoverNum [data-filter-action="filter"]',
                function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    self.filterByNumber();
            ...
            $(event.relatedTarget).on(
                "click",
                '#controlFilterPopoverNum [data-filter-action="clear"]',
                function (e) {
                    e.preventDefault();
                    e.stopPropagation();
    ...
    FilterWidget.prototype.displayPopoverNumber = function ($scope) {
        ...
        data = $.extend({}, data, {
            filter_button_text: this.getLang(
                "filter.numbers.filter_button_text"
            ),
            reset_button_text: this.getLang("filter.numbers.reset_button_text"),
            number_placeholder: this.getLang(
    ...
    FilterWidget.prototype.displayPopoverNumberRange = function ($scope) {
        ...
        data = $.extend({}, data, {
            ...
            reset_button_text: this.getLang("filter.numbers.reset_button_text"),
            min_placeholder: this.getLang(
                "filter.numbers.min_placeholder",
                "Min"
            ),
            max_placeholder: this.getLang(
                "filter.numbers.max_placeholder",
                "Max"
            ),
        });
        ...
        $scope.ocPopover({
            content: Mustache.render(
                this.getPopoverNumberRangeTemplate(),
                data
            ),
            modal: false,
            highlightModalTarget: true,
    ...
    FilterWidget.prototype.updateScopeNumberSetting = function (
        $scope,
        numbers
    ) {
        ...
        if (numbers && numbers.length) {
            numbers[0] =
                numbers[0] && numbers[0].match(numberRegex) ? numbers[0] : null;

            if (numbers.length > 1) {
                numbers[1] =
                    numbers[1] && numbers[1].match(numberRegex)
                        ? numbers[1]
                        : null;
