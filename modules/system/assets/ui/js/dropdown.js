...
+(function ($) {
    ...
    $(document).on("shown.bs.dropdown", ".dropdown", function (
        ...
        var dropdown = $(relatedTarget.relatedTarget).siblings(
                ".dropdown-menu"
            ),
        ...
        if ($(".dropdown-container", dropdown).length == 0) {
            ...
            dropdown.prepend(
                $("<li />").addClass("dropdown-title").text(title)
            );
            ...
            dropdown.on("click", "a", function () {
                if (dropdown.hasClass("scroll")) return false;
            });
        }
    ...
    $(document).on("shown.bs.dropdown", ".dropdown.dropdown-fixed", function (
        event,
        eventData
    ) {
        $container = $(this);
        ...
        $(window).on(
            "scroll.oc.dropdown, resize.oc.dropdown",
            fixDropdownPosition
        );
    });
