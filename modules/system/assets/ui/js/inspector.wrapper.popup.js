...
+(function ($) {
    ...
    InspectorPopup.prototype.createSurfaceAndUi = function (
        properties,
        values,
        title,
        description
        ...
        this.initSurface(
            this.$popoverContainer.find("[data-surface-container]").get(0),
            properties,
            values
        );
    ...
    InspectorPopup.prototype.showPopover = function () {
        var offset = this.$element.data("inspector-offset"),
            ...
            fallbackPlacement = this.$element.data(
                "inspector-fallback-placement"
            );
        ...
        if (this.options.containerSupported) {
            var moveToContainerButton = $(
                '<span class="inspector-move-to-container oc-icon-download">'
            );

            this.$popoverContainer
                .find(".popover-head")
                .append(moveToContainerButton);
        }

        this.$popoverContainer.find("[data-inspector-title]").text(this.title);
        this.$popoverContainer
            .find("[data-inspector-description]")
    ...
    InspectorPopup.prototype.repositionPopover = function () {
        ...
        this.$popoverContainer
            .find("div[data-surface-container] > div")
            .trigger("focus-control");
    };
    ...
    InspectorPopup.prototype.registerPopupHandlers = function () {
        this.surface.options.onPopupDisplayed = this.proxy(
            this.onPopupEditorDisplayed
        );
        this.surface.options.onPopupHidden = this.proxy(
            this.onPopupEditorHidden
        );
        this.popoverObj.options.onCheckDocumentClickTarget = this.proxy(
            this.onCheckDocumentClickTarget
        );
        ...
        if (this.options.containerSupported) {
            this.$popoverContainer.on(
                "click",
                "span.inspector-move-to-container",
                this.proxy(this.onMoveToContainer)
            );
        }
    };
    ...
    InspectorPopup.prototype.unregisterHandlers = function () {
        ...
        this.$popoverContainer.off(
            "keydown",
            this.proxy(this.onPopoverKeyDown)
        );
    ...
    InspectorPopup.prototype.onCheckDocumentClickTarget = function (element) {
        if (
            $.contains(this.$element, element) ||
            this.$element.get(0) === element
        ) {
            return true;
        }
    };
