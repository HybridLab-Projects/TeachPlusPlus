...
+(function ($) {
    ...
    if ($.oc.inspector.wrappers === undefined) $.oc.inspector.wrappers = {};
    ...
    BaseWrapper.prototype.normalizePropertyCode = function (
        code,
        configuration
    ) {
    ...
    BaseWrapper.prototype.isExternalParametersEditorEnabled = function () {
        return (
            this.$element.closest("[data-inspector-external-parameters]")
                .length > 0
        );
    };

    BaseWrapper.prototype.initSurface = function (
        containerElement,
        properties,
        values
    ) {
        ...
        options.onGetInspectableElement = this.proxy(
            this.onGetInspectableElement
        );
        ...
        this.surface = new $.oc.inspector.surface(
            ...
            properties,
            values,
            $.oc.inspector.helpers.generateElementUniqueId(
                this.$element.get(0)
            ),
            options
        );
    ...
    BaseWrapper.prototype.adoptSurface = function () {
        this.surface.options.onGetInspectableElement = this.proxy(
            this.onGetInspectableElement
        );
    };
    ...
    BaseWrapper.prototype.loadValues = function (configuration) {
        ...
        for (var i = 0, len = attributes.length; i < len; i++) {
            ...
            if ((matches = attribute.name.match(/^data-property-(.*)$/))) {
                ...
                var normalizedPropertyName = this.normalizePropertyCode(
                    matches[1],
                    configuration
                );
        ...
        if ($valuesField.length > 0) {
            ...
            for (var property in values) {
                ...
                if ($.isArray(value) || $.isPlainObject(value)) {
                    throw new Error(
                        "Inspector data-property-xxx attributes do not support complex values. Property: " +
                            property
                    );
                }
    ...
    BaseWrapper.prototype.loadConfiguration = function () {
        ...
        var $configurationField = this.$element.find(
            "> input[data-inspector-config]"
        );

        if ($configurationField.length > 0) {
            result.properties = this.parseConfiguration(
                $configurationField.val()
            );
        ...
        $form
            .request("onGetInspectorConfiguration", {
                data: data,
            })
    ...
    BaseWrapper.prototype.onConfigurartionRequestDone = function (
        data,
        result
    ) {
        result.properties = this.parseConfiguration(
