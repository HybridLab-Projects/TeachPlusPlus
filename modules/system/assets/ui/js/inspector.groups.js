...
+(function ($) {
    ...
    GroupManager.prototype.findGroupRows = function (
        table,
        index,
        ignoreCollapsedSubgroups
    ) {
    ...
    function getInspectorGroupStatuses() {
        var statuses = document.body.getAttribute(
            "data-inspector-group-statuses"
        );
    ...
    Group.prototype.findGroupRows = function (
        table,
        ignoreCollapsedSubgroups,
        groupManager
    ) {
        var groupIndex = this.getGroupIndex(),
            rows = table.querySelectorAll(
                'tr[data-parent-group-index="' + groupIndex + '"]'
            ),
        ...
        for (var i = 0, len = this.groups.length; i < len; i++) {
            ...
            if (
                ignoreCollapsedSubgroups &&
                !groupManager.isGroupExpanded(subgroup)
            ) {
                continue;
            }

            var subgroupRows = subgroup.findGroupRows(
                table,
                ignoreCollapsedSubgroups,
                groupManager
            );
            for (
                var j = 0, subgroupLen = subgroupRows.length;
                j < subgroupLen;
                j++
            ) {
    ...
    Group.prototype.findGroupRow = function (table) {
        return table.querySelector(
            'tr[data-group-index="' + this.groupIndex + '"]'
        );
    };
