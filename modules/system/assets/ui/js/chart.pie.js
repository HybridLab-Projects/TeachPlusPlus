...
+(function ($) {
    ...
    var PieChart = function (element, options) {
        ...
        Raphael($canvas.get(0), size, size, function () {
            ...
            self.paper.customAttributes.segment = function (
                startAngle,
                endAngle
            ) {
                var p1 = self.arcCoords(outerRadius, startAngle),
                ...
                path.hover(
                    function (ev) {
                        $.oc.chartUtils.showTooltip(
                            ...
                            ev.pageY,
                            $.trim(
                                $.oc.chartUtils.getLegendLabel($legend, index)
                            ) +
                                ": <strong>" +
                                valueInfo.value +
                                "</stong>"
            ...
            $.each(values.values, function (index, valueInfo) {
                var length =
                    values.total && valueInfo.value
                        ? (360 / values.total) * valueInfo.value
                        : 0;
                if (length == 360) length--;

                self.segments[index].animate(
        ...
        if (this.options.centerText !== undefined) {
            var $text = $("<span>")
                .addClass("center")
                .html(this.options.centerText);
            $canvas.append($text);
        }
    };

    PieChart.prototype.arcCoords = function (radius, angle) {
        var a = Raphael.rad(angle),
