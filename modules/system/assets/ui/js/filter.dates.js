...
+(function ($) {
    ...
    FilterWidget.prototype.initFilterDate = function () {
        ...
        this.$el.on("show.oc.popover", "a.filter-scope-date", function (event) {
            ...
            $(event.relatedTarget).on(
                "click",
                '#controlFilterPopoverDate [data-filter-action="filter"]',
                function (e) {
                    e.preventDefault();
                    e.stopPropagation();
            ...
            $(event.relatedTarget).on(
                "click",
                '#controlFilterPopoverDate [data-filter-action="clear"]',
                function (e) {
                    e.preventDefault();
                    e.stopPropagation();
    ...
    FilterWidget.prototype.displayPopoverRange = function ($scope) {
        ...
        data = $.extend({}, data, {
            ...
            after_placeholder: this.getLang(
                "filter.dates.after_placeholder",
                "After"
            ),
            before_placeholder: this.getLang(
                "filter.dates.before_placeholder",
                "Before"
            ),
        });
    ...
    FilterWidget.prototype.initDatePickers = function (isRange) {
        ...
        $inputs.each(function (index, datepicker) {
            ...
            if (0 <= index && index < data.dates.length) {
                defaultValue = data.dates[index]
                    ? moment
                          .tz(data.dates[index], self.appTimezone)
                          .tz(self.timezone)
                    : "";
            }
            ...
            datepicker.value =
                "" !== defaultValue
                    ? defaultValue.format(self.getDateFormat())
                    : "";
    ...
    FilterWidget.prototype.updateScopeDateSetting = function ($scope, dates) {
        ...
        if (dates && dates.length) {
            ...
            if (dates.length > 1) {
                dates[1] =
                    dates[1] && dates[1].match(dateRegex) ? dates[1] : null;

                if (dates[0] || dates[1]) {
                    var after = dates[0]
                            ? moment
                                  .tz(dates[0], this.appTimezone)
                                  .tz(this.timezone)
                                  .format(dateFormat)
                            : "∞",
                        before = dates[1]
                            ? moment
                                  .tz(dates[1], this.appTimezone)
                                  .tz(this.timezone)
                                  .format(dateFormat)
                            : "∞";
    ...
    FilterWidget.prototype.onCheckDocumentClickTargetDatePicker = function (
        target
    ) {
        var $target = $(target);
