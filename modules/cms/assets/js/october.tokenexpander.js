...
+(function ($) {
    ...
    TokenExpander.prototype.handleCursorOnToken = function (cursor, token) {
        var line = this.$session.getLine(cursor.row),
            definition = this.getTwigTokenDefinition(
                token,
                line,
                cursor.column
            );
        ...
        if (definition) {
            ...
            if (value) {
                if (!this.tokenName) this.$el.trigger("show.oc.tokenexpander");
    ...
    TokenExpander.prototype.getTwigTokenValue = function (
        tokenName,
        tokenString
    ) {
        ...
        if (regexMatch && regexMatch[2]) return regexMatch[2];
     ...
     */
    TokenExpander.prototype.getTwigTokenDefinition = function (
        token,
        str,
        pos,
        filter
        ...
        if (regexMatch) {
            ...
            // Win!
            if (start < pos && end > pos) return [regexMatch[0], start, end];
    ...
    $.fn.tokenExpander = function (option) {
        ...
        this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.tokenexpander",
                    (data = new TokenExpander(this, options))
                );
