...
+(function ($) {
    ...
    CmsPage.prototype.updateTemplateList = function (type) {
        ...
        $form.request(templateList + "::onUpdate", {
            complete: function () {
                $("button[data-control=delete-template]", $form).trigger(
                    "oc.triggerOn.update"
                );
            },
        });
    ...
    CmsPage.prototype.registerHandlers = function () {
        ...
        $masterTabs.on(
            ...
            this.proxy(this.onAfterAllTabsClosed)
        );
        ...
        $document.on(
            "open.oc.list",
            "#cms-side-panel",
            this.proxy(this.onOpenDocument)
        );
        $document.on(
            "ajaxUpdate",
            "[data-control=filelist], [data-control=assetlist]",
            this.proxy(this.onAjaxUpdate)
        );
        $document.on(
            "ajaxError",
            "#cms-master-tabs form",
            this.proxy(this.onAjaxError)
        );
        $document.on(
            "ajaxSuccess",
            "#cms-master-tabs form",
            this.proxy(this.onAjaxSuccess)
        );
        $document.on(
            "click",
            "#cms-side-panel form button[data-control=create-template], #cms-side-panel form li a[data-control=create-template]",
            this.proxy(this.onCreateTemplateClick)
        );
        $document.on(
            "click",
            "#cms-side-panel form button[data-control=delete-template]",
            this.proxy(this.onDeleteTemplateClick)
        );
        $document.on(
            "showing.oc.inspector",
            "[data-inspectable]",
            this.proxy(this.onInspectorShowing)
        );
        $document.on(
            "hidden.oc.inspector",
            "[data-inspectable]",
            this.proxy(this.onInspectorHidden)
        );
        $document.on(
            "hiding.oc.inspector",
            "[data-inspectable]",
            this.proxy(this.onInspectorHiding)
        );
        $document.on(
            "click",
            "#cms-master-tabs > div.tab-content > .tab-pane.active .control-componentlist a.remove",
            this.proxy(this.onComponentRemove)
        );
        $document.on(
            "click",
            "#cms-component-list [data-component]",
            this.proxy(this.onComponentClick)
        );
    };
    ...
    CmsPage.prototype.onOpenDocument = function (event) {
         ...
         */
        if ($("#cms-master-tabs").data("oc.tab").goTo(tabId)) return false;
    ...
    CmsPage.prototype.ajaxInvalidField = function (
        ev,
        element,
        name,
        messages,
        ...
        if ($panel.length > 0) $panel.removeClass("collapsed");
    ...
    CmsPage.prototype.onTabClosed = function (ev) {
        ...
        if (
            $("> div.tab-content > div.tab-pane", "#cms-master-tabs").length ==
            0
        )
            this.setPageTitle("");
    };
    ...
    CmsPage.prototype.onBeforeRequest = function (ev) {
        ...
        if ($(".components .layout-cell.error-component", $form).length > 0) {
            if (
                !confirm(
                    "The form contains unknown components. Their properties will be lost on save. Do you want to save the form?"
                )
            )
                ev.preventDefault();
        }
    ...
    CmsPage.prototype.onTabShown = function (ev) {
        ...
        if (
            $target.closest("[data-control=tab]").attr("id") !=
            "cms-master-tabs"
        )
            return;
        ...
        $sidePanel
            .find("[data-control=filelist]")
            .fileList("markActive", dataId);
    ...
    CmsPage.prototype.onInitTab = function (ev, data) {
        ...
        if ($(ev.target).attr("id") != "cms-master-tabs") return;

        var $collapseIcon = $(
                '<a href="javascript:;" class="tab-collapse-icon tabless"><i class="icon-chevron-up"></i></a>'
            ),
        ...
        $collapseIcon.click(function () {
            ...
            if (typeof localStorage !== "undefined")
                localStorage.ocCmsTablessCollapsed = $panel.hasClass(
                    "collapsed"
                )
                    ? 1
                    : 0;
        ...
        var $primaryCollapseIcon = $(
                '<a href="javascript:;" class="tab-collapse-icon primary"><i class="icon-chevron-down"></i></a>'
            ),
        ...
        if ($primaryPanel.length > 0) {
            ...
            $primaryCollapseIcon.click(function () {
                ...
                $(window).trigger("oc.updateUi");
                if (typeof localStorage !== "undefined")
                    localStorage.ocCmsPrimaryCollapsed = $primaryPanel.hasClass(
                        "collapsed"
                    )
                        ? 1
                        : 0;
        ...
        if (typeof localStorage !== "undefined") {
            if (
                !$("a", data.tab).hasClass("new-template") &&
                localStorage.ocCmsTablessCollapsed == 1
            )
        ...
        var $componentListFormGroup = $(
            ".control-componentlist",
            data.pane
        ).closest(".form-group");
        if ($primaryPanel.length > 0)
            $primaryPanel.before($componentListFormGroup);
        else $secondaryPanel.parent().before($componentListFormGroup);
    ...
    CmsPage.prototype.onAjaxUpdate = function (ev) {
        var dataId = $("#cms-master-tabs .nav-tabs li.active").attr(
                "data-tab-id"
            ),
            $sidePanel = $("#cms-side-panel");
    ...
    CmsPage.prototype.onAjaxSuccess = function (ev, context, data) {
        ...
        $("[data-control=commit-button]", element).toggleClass(
            "hide",
            !data.canCommit
        );
        $("[data-control=reset-button]", element).toggleClass(
            "hide",
            !data.canReset
        );
        ...
        if (data.templatePath !== undefined) {
            ...
            if (data.pageUrl !== undefined)
                $("[data-control=preview-button]", element).attr(
                    "href",
                    data.pageUrl
                );
        }
        ...
        var tabId =
            $("input[name=templateType]", element).val() +
            "-" +
        ...
        if (templateType.length > 0) {
            ...
            if (templateType == "layout") this.updateLayouts(element);
        }
    ...
    CmsPage.prototype.onAjaxError = function (
        ev,
        context,
        message,
        data,
        ...
        $form
            ...
            .done(function (data) {
                $("#cms-master-tabs").ocTab(
                    ...
                    data.tab,
                    tabId,
                    $form.data("type-icon") + " new-template"
                );
    ...
    CmsPage.prototype.onDeleteTemplateClick = function (ev) {
        ...
        if (!confirm($el.data("confirmation"))) return;

        $.oc.stripeLoadIndicator.show();

        $form
            .request("onDeleteTemplates", {
                data: { type: templateType },
            })
            .done(function (data) {
                var tabs = $("#cms-master-tabs").data("oc.tab");
                $.each(data.deleted, function (index, path) {
                    var tabId = templateType + "-" + data.theme + "-" + path,
    ...
    CmsPage.prototype.onInspectorShowing = function (ev, data) {
        var $dragScroll = $(ev.currentTarget)
            .closest('[data-control="toolbar"]')
            .data("oc.dragScroll");
        if ($dragScroll) {
    ...
    CmsPage.prototype.onInspectorHiding = function (ev, values) {
        var element = ev.target,
            ...
            $componentList = $(
                "#cms-master-tabs > div.tab-content > .tab-pane.active .control-componentlist .layout"
            ),
            $cell = $(ev.target).parent();

        $("div.layout-cell", $componentList).each(function () {
            if ($cell.get(0) == this) return true;
    ...
    CmsPage.prototype.onComponentClick = function (ev) {
        ...
        if ($componentList.length == 0) {
            alert(
                "Components can be added only to pages, partials and layouts."
            );
            return;
        }
        ...
        $(
            'div.layout-cell input[name="component_aliases[]"]',
            $componentList
        ).each(function () {
        ...
        $componentContainer.attr({
            ...
            "data-inspector-description": $component
    ...
    CmsPage.prototype.updateComponentListClass = function (pane) {
        var $componentList = $(".control-componentlist", pane),
            ...
            hasComponents =
                $(".layout", $componentList).children(":not(.hidden)").length >
                0;
    ...
    CmsPage.prototype.updateFormEditorMode = function (pane, initialization) {
        ...
        if ($contentTypeElement.length == 0) return;

        if ($contentTypeElement.data("toolbar-type") != "content") return;
        ...
        if (parts.length >= 2) extension = parts.pop().toLowerCase();

        if (modes[extension] !== undefined) mode = modes[extension];
        ...
        if (initialization) editor.on("oc.codeEditorReady", setEditorMode);
        else setEditorMode();
    };
    ...
    CmsPage.prototype.updateModifiedCounter = function () {
        ...
        $(
            "> div.tab-content > div.tab-pane[data-modified]",
            "#cms-master-tabs"
        ).each(function () {
    ...
    CmsPage.prototype.addTokenExpanderToEditor = function (pane, $form) {
        ...
        if (!editor.length || editor.data("oc.tokenexpander")) return;
        ...
        var breakButton = $("<li />")
            .prop({ class: "tokenexpander-button" })
            .append(
                $("<a />")
                    .prop({ href: "javascript:; " })
                    .append($("<i />").prop({ class: "icon-code-fork" }))
            );
    ...
    CmsPage.prototype.setPageTitle = function (title) {
        if (title.length) $.oc.layout.setPageTitle(title + " | ");
        else $.oc.layout.setPageTitle(title);
    };
    ...
    CmsPage.prototype.updateLayouts = function (form) {
        $(form).request("onGetTemplateList", {
            success: function (data) {
                $(
                    '#cms-master-tabs > .tab-content select[name="settings[layout]"]'
                ).each(function () {
                    var $select = $(this),
                        value = $select.val();
