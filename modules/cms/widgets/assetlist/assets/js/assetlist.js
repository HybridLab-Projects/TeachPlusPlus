...
+(function ($) {
    ...
    var AssetList = function (form, alias) {
        ...
        this.$form.on(
            "click",
            "ul.list > li.directory > a",
            $.proxy(this.onDirectoryClick, this)
        );
        this.$form.on(
            "click",
            "ul.list > li.file > a",
            $.proxy(this.onFileClick, this)
        );
        this.$form.on(
            "click",
            "p.parent > a",
        ...
        this.$form.on(
            "click",
            "a[data-control=delete-asset]",
            $.proxy(this.onDeleteClick, this)
        );
        this.$form.on(
            "oc.list.setActiveItem",
            $.proxy(this.onSetActiveItem, this)
        );
    ...
    AssetList.prototype.gotoDirectory = function (path, gotoParent) {
        ...
        else $container.addClass("goForward");

        $.oc.stripeLoadIndicator.show();
        this.$form
            .request(this.alias + "::onOpenDirectory", {
                data: {
                    path: path,
                    d: 0.2,
                },
    ...
    AssetList.prototype.onDeleteClick = function (e) {
        ...
        if (!confirm($el.data("confirmation"))) return false;
    ...
    AssetList.prototype.onSetActiveItem = function (event, dataId) {
        ...
        if (dataId)
            $('ul li.file[data-id="' + dataId + '"]', this.$form).addClass(
                "active"
            );
    };
    ...
    AssetList.prototype.updateUi = function () {
        $("button[data-control=asset-tools]", self.$form).trigger(
            "oc.triggerOn.update"
        );
    };
    ...
    $(document).ready(function () {
        new AssetList(
            $("#asset-list-container").closest("form"),
            $("#asset-list-container").data("alias")
        );
    });
})(window.jQuery);
