...
+(function ($) {
    ...
    TreeView.prototype.init = function () {
        ...
        this.$el.on(
            "click.treeview",
            "li > div > ul.submenu li a",
            this.proxy(this.onOpenSubmenu)
        );
        this.$el.on("click.treeview", "li > div > a", this.proxy(this.onOpen));
        this.$el.on(
            "click.treeview",
            "li span.expand",
            this.proxy(this.onItemExpandClick)
        );
    ...
    TreeView.prototype.createItemControls = function () {
        $("li", this.$el).each(function () {
            ...
            if ($(this).attr("data-no-drag-mode") !== undefined)
                $("span.drag-handle", this).attr(
                    "title",
                    "Dragging is disabled when the Search is active"
                );
        });
    };
    ...
    TreeView.prototype.expandGroup = function ($group) {
        ...
        $subitems.animate(
            { height: $subitems[0].scrollHeight },
            {
                duration: 100,
                queue: false,
                complete: function () {
    ...
    TreeView.prototype.sendReorderRequest = function () {
        if (this.options.reorderHandler === undefined) return;
        ...
        this.$el.request(this.options.reorderHandler, {
            data: { structure: JSON.stringify(groups) },
        });
    };
    ...
    TreeView.prototype.initSortable = function () {
        ...
        if ($noDragItems.length > 0) return;

        if (this.$el.data("oc.treelist")) this.$el.treeListWidget("unbind");
    ...
    TreeView.prototype.onOpenSubmenu = function (ev) {
        var e = $.Event("submenu.oc.treeview", {
    ...
    TreeView.prototype.onOpen = function (ev) {
        var e = $.Event("open.oc.treeview", {
            relatedTarget: $(ev.currentTarget).closest("li").get(0),
            clickEvent: ev,
        });
    ...
    TreeView.prototype.onDrag = function ($item, position, _super, event) {
        ...
        if (!this.$scrollbar || this.$scrollbar.length === 0) return;

        if (position.top < 0) {
            this.scrollOffset = -10 + Math.floor(position.top / 5);
        } else if (position.top > this.$scrollbar.height()) {
            this.scrollOffset =
                10 + Math.ceil((position.top - this.$scrollbar.height()) / 5);
        } else {
            return;
        }
    ...
    TreeView.prototype.dragScroll = function () {
        ...
        this.$scrollbar.scrollTop(
            Math.min(startScrollTop + this.scrollOffset, this.scrollMax())
        );
    ...
    $.fn.treeView = function (option) {
        ...
        return this.each(function () {
            ...
            if (typeof option == "string" && data) {
                ...
                if (data[option]) data[option].apply(data, methodArgs);
            }
        });
    };
