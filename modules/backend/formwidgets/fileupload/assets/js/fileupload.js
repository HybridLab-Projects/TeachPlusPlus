...
+(function ($) {
    ...
    FileUpload.prototype.init = function () {
        ...
        this.$el.on(
            ...
            this.proxy(this.onClickSuccessObject)
        );
        this.$el.on(
            "click",
            ".upload-object.is-error",
            this.proxy(this.onClickErrorObject)
        );

        // Stop here for preview mode
        if (this.options.isPreview) return;

        this.$el.on(
            "click",
            ".upload-remove-button",
            this.proxy(this.onRemoveObject)
        );
    ...
    FileUpload.prototype.bindUploader = function () {
        ...
        if (this.options.template) {
            this.uploaderOptions.previewTemplate = $(
    ...
    FileUpload.prototype.onResizeFileInfo = function (file) {
        var info, targetWidth, targetHeight;
        ...
        if (!this.options.thumbnailWidth && !this.options.thumbnailWidth) {
            ...
            targetWidth =
                (this.options.thumbnailHeight * file.height) / file.width;
    ...
    FileUpload.prototype.onUploadAddedFile = function (file) {
        ...
        $(file.previewElement)
            .find("[data-dz-size]")
            .html("<strong>" + filesize.size + "</strong> " + filesize.units);
    ...
    FileUpload.prototype.bindSortable = function () {
        var self = this,
            placeholderEl = $(
                '<div class="upload-object upload-placeholder"/>'
            ).css({
    ...
    FileUpload.prototype.onSortAttachments = function () {
        if (this.options.sortHandler) {
            ...
            this.$el.find(".upload-object.is-success").each(function (index) {
                var id = $(this).data("id");
    ...
    FileUpload.prototype.onClickSuccessObject = function (ev) {
        ...
        $target.one("popupComplete", function (event, element, modal) {
            modal.one("ajaxDone", "button[type=submit]", function (
                e,
                context,
                data
            ) {
