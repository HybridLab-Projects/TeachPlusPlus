(function ($) {
            ...
            new $.oc.mediaManager.popup({
                ...
                onInsert: function (items) {
                    if (!items.length) {
                        $.oc.alert(
                            $.oc.lang.get(
                                "mediamanager.invalid_file_empty_insert"
                            )
                        );
                        return;
                    }
                    if (items.length > 1) {
                        $.oc.alert(
                            $.oc.lang.get(
                                "mediamanager.invalid_file_single_insert"
                            )
                        );
                        return;
                    }
            ...
            new $.oc.mediaManager.popup({
                ...
                onInsert: function (items) {
                    ...
                    if (!items.length) {
                        $.oc.alert(
                            $.oc.lang.get(
                                "mediamanager.invalid_image_empty_insert"
                            )
                        );
                        return;
                    }
            ...
            new $.oc.mediaManager.popup({
                ...
                onInsert: function (items) {
                    if (!items.length) {
                        $.oc.alert(
                            $.oc.lang.get(
                                "mediamanager.invalid_video_empty_insert"
                            )
                        );
                        return;
                    }
                    ...
                    if (item.documentType !== "video") {
                        $.oc.alert(
                            $.oc.lang.get(
                                "mediamanager.invalid_video_invalid_insert",
                                'The file "' + item.title + '" is not a video.'
                            )
                        );
                        return;
                    }
                    var $richEditorNode = editor.$el.closest(
                        '[data-control="richeditor"]'
                    );
                    $richEditorNode.richEditor(
                        "insertVideo",
                        item.publicUrl,
                        item.title
                    );
            ...
            new $.oc.mediaManager.popup({
                ...
                onInsert: function (items) {
                    if (!items.length) {
                        $.oc.alert(
                            $.oc.lang.get(
                                "mediamanager.invalid_audio_empty_insert"
                            )
                        );
                        return;
                    }
                    ...
                    if (item.documentType !== "audio") {
                        $.oc.alert(
                            $.oc.lang.get(
                                "mediamanager.invalid_audio_invalid_insert",
                                'The file "' +
                                    item.title +
    ...
    if (
        ...
        throw new Error(
            "Media manager plugin requires link, file, image and video plugin."
        );
    }
...
$.FroalaEditor.DEFAULTS.key =
    "JA6B2B5A1qB1F1F4D3I1A15A11D3E6B5dVh1VCQWa1EOQFe1NCb1==";
(function ($) {
    $.FroalaEditor.PLUGINS.pageLinks = function (editor) {
        ...
        function setLinkValue(link) {
            ...
            var check_inputs = $popup.find(
                'input.fr-link-attr[type="checkbox"]'
            );
            var $input;
            var i;
            ...
            for (i = 0; i < check_inputs.length; i++) {
                ...
                $input.prop(
                    "checked",
                    $input.data("checked") == link[$input.attr("name")]
                );
            }
        }
        function insertLink() {
...
(function ($) {
    $.FroalaEditor.PLUGINS.figures = function (editor) {
        ...
        function _makeUiBlockElement() {
            var $node = $(
                '<figure contenteditable="false" tabindex="0" data-ui-block="true">&nbsp;</figure>'
            );
        ...
        function _handleUiBlockCaretClearEmpty($block, $p) {
            if (
                $block.attr("data-ui-block") !== undefined &&
                $.trim($p.text()).length == 0
            ) {
                $p.remove();
        ...
        function _onSync(html) {
            ...
            $domTree.find("[data-video], [data-audio]").each(function () {
                $(this)
                    .removeAttr(
                        "contenteditable data-ui-block tabindex draggable"
                    )
...
+(function ($) {
    ...
    RichEditor.prototype.initFroala = function () {
        ...
        froalaOptions.linkStyles = this.options.linkStyles
            ...
            : { "oc-link-green": "Green", "oc-link-strong": "Thick" };
        ...
        if (this.options.htmlAllowedEmptyTags) {
            froalaOptions.allowEmptyTags = this.options.htmlAllowedEmptyTags.split(
                /[\s,]+/
            );
        }
        if (this.options.allowTags) {
            froalaOptions.htmlAllowedTags = this.options.allowTags.split(
                /[\s,]+/
            );
        }
        ...
        if (this.options.removeTags) {
            froalaOptions.htmlRemoveTags = this.options.removeTags.split(
                /[\s,]+/
            );
        }
        ...
        froalaOptions.shortcutsEnabled = [
            "show",
            "bold",
            "italic",
            "underline",
        ...
        froalaOptions.imageUploadURL = froalaOptions.fileUploadURL =
            window.location;
        froalaOptions.imageUploadParam = froalaOptions.fileUploadParam =
            "file_data";
        ...
        this.$textarea.on(
            ...
            this.proxy(this.onChange)
        );
        this.$textarea.on(
            "froalaEditor.html.get",
            this.proxy(this.onSyncContent)
        );
        this.$textarea.on(
            "froalaEditor.html.set",
            this.proxy(this.onSetContent)
        );
        ...
        this.$el.on(
            "keydown",
            ".fr-view figure",
            this.proxy(this.onFigureKeydown)
        );
    };
    ...
    $.fn.richEditor = function (option) {
        ...
        this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.richEditor",
                    (data = new RichEditor(this, options))
                );
