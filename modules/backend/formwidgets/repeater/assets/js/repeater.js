...
+(function ($) {
    ...
    Repeater.prototype.init = function () {
        ...
        this.$el.on(
            "ajaxDone",
            "> .field-repeater-items > .field-repeater-item > .repeater-item-remove > [data-repeater-remove]",
            this.proxy(this.onRemoveItemSuccess)
        );
        this.$el.on(
            "ajaxDone",
            "> .field-repeater-add-item > [data-repeater-add]",
            this.proxy(this.onAddItemSuccess)
        );
        this.$el.on(
            "click",
            "> ul > li > .repeater-item-collapse .repeater-item-collapse-one",
            this.proxy(this.toggleCollapse)
        );
        this.$el.on(
            "click",
            "> .field-repeater-add-item > [data-repeater-add-group]",
            this.proxy(this.clickAddGroupButton)
        );
    ...
    Repeater.prototype.dispose = function () {
        ...
        this.$el.off(
            "click",
            "> .field-repeater-items > .field-repeater-item > .repeater-item-collapse .repeater-item-collapse-one",
            this.proxy(this.toggleCollapse)
        );
        this.$el.off(
            "click",
    ...
    Repeater.prototype.onRemoveItemSuccess = function (ev) {
        ...
        $(ev.target)
            .closest(".field-repeater-item")
            .find("[data-disposable]")
            .each(function () {
                var $elem = $(this),
    ...
    Repeater.prototype.togglePrompt = function () {
        if (this.options.minItems && this.options.minItems > 0) {
            var repeatedItems = this.$el.find(
                    "> .field-repeater-items > .field-repeater-item"
                ).length,
                $removeItemBtn = this.$el.find(
                    "> .field-repeater-items > .field-repeater-item > .repeater-item-remove"
                );

            $removeItemBtn.toggleClass(
                "disabled",
                !(repeatedItems > this.options.minItems)
            );
        }
    ...
    Repeater.prototype.collapse = function ($item) {
        ...
        $(".repeater-item-collapsed-title", $item).text(
            this.getCollapseTitle($item)
        );
    };
    ...
    Repeater.prototype.getCollapseTitle = function ($item) {
        ...
        if (this.options.titleFrom) {
            $target = $(
                '[data-field-name="' + this.options.titleFrom + '"]',
                $item
            );
            if (!$target.length) {
                $target = $item;
        ...
        var $textInput = $(
            "input[type=text]:first, select:first",
            $target
        ).first();
        if ($textInput.length) {
            ...
            var $disabledTextInput = $(
                ".text-field:first > .form-control",
                $target
            );
    ...
    Repeater.prototype.getStyle = function () {
        ...
        if (
            this.options.style &&
            ["collapsed", "accordion"].indexOf(this.options.style) !== -1
        ) {
