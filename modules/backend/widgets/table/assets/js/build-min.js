+(function ($) {
    "use strict";
    if ($.oc === undefined) $.oc = {};
    if ($.oc.table === undefined) $.oc.table = {};
    var Table = function (element, options) {
        ...
        if (
            this.options.postback &&
            this.options.clientDataSourceClass == "client"
        )
    ...
    Table.prototype.createDataSource = function () {
        ...
        if (
            $.oc.table.datasource === undefined ||
            $.oc.table.datasource[dataSourceClass] == undefined
        )
            throw new Error(
                'The table client-side data source class "' +
                    dataSourceClass +
                    '" is not ' +
    ...
    Table.prototype.registerHandlers = function () {
        ...
        if (
            ...
            this.$el
                .closest("form")
                .bind("oc.beforeRequest", this.formSubmitHandler);
    ...
    Table.prototype.unregisterHandlers = function () {
        ...
        if (this.formSubmitHandler) {
            this.$el
                .closest("form")
                .unbind("oc.beforeRequest", this.formSubmitHandler);
    ...
    Table.prototype.initCellProcessors = function () {
        for (var i = 0, len = this.options.columns.length; i < len; i++) {
            ...
            if (
                $.oc.table.processor === undefined ||
                $.oc.table.processor[columnType] == undefined
            )
                throw new Error(
                    'The table cell processor for the column type "' +
                        columnType +
                        '" is not ' +
                        "found in the $.oc.table.processor namespace."
                );
            this.cellProcessors[column] = new $.oc.table.processor[columnType](
                this,
                column,
                columnConfiguration
            );
    ...
    Table.prototype.buildToolbar = function () {
        ...
        this.toolbar = $($("[data-table-toolbar]", this.el).html())
        ...
        if (!this.options.adding) {
            ...
            if (
                this.navigation.paginationEnabled() ||
                !this.options.rowSorting
            ) {
                $(
                    "[data-cmd=record-add-below], [data-cmd=record-add-above]",
                    this.toolbar
                ).remove();
            } else {
                $("[data-cmd=record-add]", this.toolbar).remove();
    ...
    Table.prototype.buildHeaderTable = function () {
        ...
        for (var i = 0, len = this.options.columns.length; i < len; i++) {
            ...
            if (this.options.columns[i].width)
                header.setAttribute(
                    "style",
                    "width: " + this.options.columns[i].width
                );
    ...
    Table.prototype.updateDataTable = function (onSuccess) {
        var self = this;
        this.unfocusTable();
        this.fetchRecords(function onUpdateDataTableSuccess(
            records,
            totalCount
        ) {
    ...
    Table.prototype.updateColumnWidth = function () {
        ...
        for (var i = 0, len = headerCells.length; i < len; i++) {
            if (dataCells[i])
                dataCells[i].setAttribute(
                    "style",
                    headerCells[i].getAttribute("style")
                );
        }
    };
        ...
        for (var i = 0, len = records.length; i < len; i++) {
            ...
            if (records[i][keyColumn] === undefined)
                throw new Error(
                    "The row attribute " +
                        keyColumn +
                        " is not set for the row #" +
                        i
                );
            row.setAttribute("data-row", records[i][keyColumn]);
            for (
                var j = 0, colsLen = this.options.columns.length;
                j < colsLen;
                j++
            ) {
                ...
                dataContainer.value = this.formatDataContainerValue(
    ...
    Table.prototype.scrollCellIntoView = function () {
        if (!this.options.height || !this.activeCell) return;
        $(this.dataTableContainer.parentNode)
            .data("oc.scrollbar")
            .gotoElement(this.activeCell);
    };
    ...
    Table.prototype.commitEditedRow = function () {
        if (this.editedRowKey === null) return;
        var editedRow = this.dataTable.querySelector(
            'tr[data-row="' + this.editedRowKey + '"]'
        );
        if (!editedRow) return;
    ...
    Table.prototype.addRecord = function (placement, noFocus) {
        ...
        if (this.navigation.paginationEnabled()) {
            var newPageIndex = this.navigation.getNewRowPage(
                placement,
                currentRowIndex
            );
        ...
        this.dataSource.createRecord(
            ...
            function onAddRecordDataTableSuccess(records, totalCount) {
                ...
                if (!row)
                    throw new Error(
                        "New row is not found in the updated table: " +
                            recordData[keyColumn]
                    );
    ...
    Table.prototype.deleteRecord = function () {
        ...
        if (paginationEnabled)
            this.navigation.pageIndex = this.navigation.getPageAfterDeletion(
                currentRowIndex
            );
        this.recordsAddedOrDeleted++;
        var keyColumn = this.options.keyColumn,
        ...
        this.dataSource.deleteRecord(
            ...
            function onDeleteRecordDataTableSuccess(records, totalCount) {
                self.buildDataTable(records, totalCount);
                if (!paginationEnabled)
                    self.navigation.focusCellInReplacedRow(
                        currentRowIndex,
                        currentCellIndex
                    );
                else {
    ...
    Table.prototype.notifyRowProcessorsOnChange = function (cellElement) {
        ...
        for (var i = 0, len = row.children.length; i < len; i++) {
            var column = this.options.columns[i].key;
            this.cellProcessors[column].onRowValueChanged(
                columnName,
                row.children[i]
            );
        }
        ...
        for (var i = 0, rowsLen = rows.length; i < rowsLen; i++) {
            ...
            for (var columnName in rowData) {
                var cellProcessor = this.getCellProcessor(columnName),
                    message = cellProcessor.validate(
                        rowData[columnName],
                        rowData
                    );
    ...
    Table.prototype.onFormSubmit = function (ev, data) {
        if (data.handler == this.options.postbackHandlerName) {
            ...
            var fieldName =
                this.options.fieldName.indexOf("[") > -1
    ...
    Table.prototype.onDocumentClick = function (ev) {
        ...
        if (
            this.activeCellProcessor &&
            this.activeCellProcessor.elementBelongsToProcessor(target)
        )
            return;
        this.unfocusTable();
    };
    ...
    Table.prototype.elementAddClass = function (el, className) {
        if (this.elementHasClass(el, className)) return;
    ...
    Table.prototype.setCellValue = function (
        cellElement,
        value,
        suppressEvents
    ) {
...
+(function ($) {
    ...
    if ($.oc.table.helper === undefined) $.oc.table.helper = {};
    ...
    Navigation.prototype.paginationEnabled = function () {
        return (
            this.tableObj.options.recordsPerPage != null &&
    ...
    Navigation.prototype.buildPagination = function (recordCount) {
        ...
        var paginationContainer = this.tableObj
                .getElement()
    ...
    Navigation.prototype.calculatePageCount = function (
        recordCount,
        recordsPerPage
    ) {
    ...
    Navigation.prototype.getRecordCount = function (paginationContainer) {
        var container = paginationContainer
            ? paginationContainer
            : this.tableObj.getElement().querySelector(".pagination");
        return parseInt(container.getAttribute("data-record-count"));
    };
    Navigation.prototype.buildPaginationLinkList = function (
        recordCount,
        recordsPerPage,
        pageIndex
    ) {
    ...
    Navigation.prototype.markActiveLinkItem = function (
        paginationContainer,
        pageIndex
    ) {
    ...
    Navigation.prototype.navigateDown = function (ev, forceCellIndex) {
        if (!this.tableObj.activeCell) return;
        if (
            this.tableObj.activeCellProcessor &&
            !this.tableObj.activeCellProcessor.keyNavigationAllowed(ev, "down")
        )
            return;
    ...
    Navigation.prototype.navigateUp = function (ev, forceCellIndex, isTab) {
        ...
        if (
            this.tableObj.activeCellProcessor &&
            !this.tableObj.activeCellProcessor.keyNavigationAllowed(ev, "up")
        )
            return;
        var row = this.tableObj.activeCell.parentNode,
            newRow =
    ...
    Navigation.prototype.navigateLeft = function (ev, isTab) {
        ...
        if (
            !isTab &&
            this.tableObj.activeCellProcessor &&
            !this.tableObj.activeCellProcessor.keyNavigationAllowed(ev, "left")
        )
            return;
        var row = this.tableObj.activeCell.parentNode,
            newIndex =
    ...
    Navigation.prototype.navigateRight = function (ev, isTab) {
        ...
        if (
            !isTab &&
            this.tableObj.activeCellProcessor &&
            !this.tableObj.activeCellProcessor.keyNavigationAllowed(ev, "right")
        )
            return;
        var row = this.tableObj.activeCell.parentNode,
    ...
    Navigation.prototype.navigateNext = function (ev) {
        ...
        if (
            this.tableObj.activeCellProcessor &&
            !this.tableObj.activeCellProcessor.keyNavigationAllowed(ev, "tab")
        )
            return;
    ...
    Navigation.prototype.focusCellInReplacedRow = function (
        rowIndex,
        cellIndex
    ) {
        if (rowIndex == 0) {
    ...
    Navigation.prototype.onKeydown = function (ev) {
        if (ev.key === "ArrowDown") return this.navigateDown(ev);
...
+(function ($) {
    ...
    Search.prototype.buildSearchForm = function () {
        ...
        if (!searchForm) {
            this.searchForm = $($("[data-table-toolbar-search]", el).html())
                .appendTo(toolbar)
                .get(0);
...
+(function ($) {
    ...
    if ($.oc.table.datasource === undefined)
        throw new Error(
            "The $.oc.table.datasource namespace is not defined. Make sure that the table.datasource.base.js script is loaded."
        );
    ...
    var Client = function (tableObj) {
        ...
        if (dataString === null || dataString === undefined)
            throw new Error(
                "The required data-data attribute is not found on the table control element."
            );
    ...
    Client.prototype.getRecords = function (offset, count, onSuccess) {
        if (!count) {
            ...
            onSuccess(
                this.data.slice(offset, offset + count),
                this.data.length
            );
        }
    };
    Client.prototype.createRecord = function (
        recordData,
        placement,
        relativeToKey,
        offset,
    ...
    Client.prototype.deleteRecord = function (
        key,
        newRecordData,
        offset,
        count,
...
+(function ($) {
    ...
    Server.prototype.getRecords = function (offset, count, onSuccess) {
        ...
        this.tableObj.$el
            .request(handlerName, { data: { offset: offset, count: count } })
            .done(function (data) {
                onSuccess(data.records, data.count);
            });
    };
    Server.prototype.searchRecords = function (
        query,
        offset,
        count,
        onSuccess
        ...
        this.tableObj.$el
            .request(handlerName, {
                data: { query: query, offset: offset, count: count },
            })
            .done(function (data) {
                onSuccess(data.records, data.count);
            });
    };
    Server.prototype.createRecord = function (
        recordData,
        placement,
        relativeToKey,
        offset,
    ...
    Server.prototype.updateRecord = function (key, recordData) {
        ...
        this.tableObj.$el.request(handlerName, {
            data: { key: key, recordData: recordData },
        });
    };
    Server.prototype.deleteRecord = function (
        key,
        newRecordData,
        offset,
        count,
        ...
        this.tableObj.$el
            .request(handlerName, {
                data: { key: key, offset: offset, count: count },
            })
            .done(function (data) {
                onSuccess(data.records, data.count);
            });
...
+(function ($) {
    ...
    if ($.oc.table.processor === undefined) $.oc.table.processor = {};
    ...
    Base.prototype.createViewContainer = function (
        cellContentContainer,
        value
    ) {
    ...
    Base.prototype.initValidators = function () {
        if (this.columnConfiguration.validation === undefined) return;
        for (var validatorName in this.columnConfiguration.validation) {
            if (
                $.oc.table.validator === undefined ||
                $.oc.table.validator[validatorName] == undefined
            )
                throw new Error(
                    'The table cell validator "' +
                        validatorName +
                        '" for the column "' +
                        this.columnName +
                        '" is not ' +
...
+(function ($) {
    ...
    StringProcessor.prototype.renderCell = function (
        value,
        cellContentContainer
    ) {
    ...
    StringProcessor.prototype.onFocus = function (cellElement, isClick) {
        ...
        this.buildEditor(
            cellElement,
            this.getCellContentContainer(cellElement)
        );
    };
    ...
    StringProcessor.prototype.buildEditor = function (
        cellElement,
        cellContentContainer
    ) {
    ...
    StringProcessor.prototype.onRowValueChanged = function (
        columnName,
        cellElement
    ) {
...
+(function ($) {
    ...
    var Base = $.oc.table.processor.base,
        BaseProto = Base.prototype;
    var CheckboxProcessor = function (
        tableObj,
        columnName,
        columnConfiguration
    ) {
    ...
    CheckboxProcessor.prototype.renderCell = function (
        value,
        cellContentContainer
    ) {
    ...
    CheckboxProcessor.prototype.onRowValueChanged = function (
        columnName,
        cellElement
    ) {
        if (columnName != this.columnName) {
...
+(function ($) {
    ...
    var Base = $.oc.table.processor.base,
        BaseProto = Base.prototype;
    var DropdownProcessor = function (
        tableObj,
        columnName,
        columnConfiguration
    ) {
    ...
    DropdownProcessor.prototype.unregisterListHandlers = function () {
        if (this.itemListElement) {
            ...
            this.itemListElement.removeEventListener(
                "keydown",
                this.itemKeyDownHandler
            );
            this.itemListElement.removeEventListener(
                "mousemove",
                this.itemMouseMoveHandler
            );
        }
    };
    DropdownProcessor.prototype.renderCell = function (
        value,
        cellContentContainer
    ) {
        var viewContainer = this.createViewContainer(
            cellContentContainer,
            "..."
        );
        this.fetchOptions(
            cellContentContainer.parentNode,
            function renderCellFetchOptions(options) {
    ...
    DropdownProcessor.prototype.buildEditor = function (
        cellElement,
        cellContentContainer,
        isClick
    ) {
        ...
        this.itemListElement.setAttribute(
            "class",
            "table-control-dropdown-list"
        );
        this.itemListElement.style.width =
            cellContentContainer.offsetWidth + "px";
        this.itemListElement.style.left = containerPosition.left + "px";
        this.itemListElement.style.top =
            containerPosition.top -
            2 +
            cellContentContainer.offsetHeight +
            "px";
        this.fetchOptions(cellElement, function renderCellFetchOptions(
            options
        ) {
    ...
    DropdownProcessor.prototype.hideDropdown = function () {
        if (
            ...
            this.activeCell &&
            this.itemListElement.parentNode
        ) {
            var cellContentContainer = this.getCellContentContainer(
                this.activeCell
            );
    ...
    DropdownProcessor.prototype.showDropdown = function () {
        if (
            this.itemListElement &&
            this.itemListElement.parentNode !== document.body
        ) {
            this.getCellContentContainer(this.activeCell).setAttribute(
                "data-dropdown-open",
                "true"
            );
            document.body.appendChild(this.itemListElement);
            var activeItemElement = this.itemListElement.querySelector(
                "ul li.selected"
            );
            if (!activeItemElement) {
                activeItemElement = this.itemListElement.querySelector(
                    "ul li:first-child"
                );
                if (activeItemElement)
    ...
    DropdownProcessor.prototype.fetchOptions = function (
        cellElement,
        onSuccess
    ) {
        if (this.columnConfiguration.options) {
            ...
            if (!this.cachedOptionPromises[cachingKey]) {
                var requestData = {
                    ...
                    handlerName =
                        this.tableObj.getAlias() + "::onGetDropdownOptions";
                this.cachedOptionPromises[
                    cachingKey
                ] = this.tableObj.$el.request(handlerName, {
                    data: requestData,
    ...
    DropdownProcessor.prototype.setSelectedItem = function (item) {
        if (!this.itemListElement) return null;
    ...
    DropdownProcessor.prototype.onKeyDown = function (ev) {
        ...
        if (
            ...
            if (!selected) {
                ...
                newSelectedItem = this.itemListElement.querySelector(
                    "ul li:first-child"
                );
            } else {
    ...
    DropdownProcessor.prototype.onRowValueChanged = function (
        columnName,
        cellElement
    ) {
        if (!this.columnConfiguration.dependsOn) return;
        ...
        if (!dependsOnColumn) return;
        var currentValue = this.tableObj.getCellValue(cellElement),
            viewContainer = this.getViewContainer(cellElement);
        this.fetchOptions(cellElement, function rowValueChangedFetchOptions(
            options
        ) {
            var value =
    ...
    DropdownProcessor.prototype.elementBelongsToProcessor = function (element) {
        if (!this.itemListElement) return false;
        return this.tableObj.parentContainsElement(
            this.itemListElement,
            element
        );
    };
        ...
        if (character.length === 1 || character === "Space") {
            ...
            if (validItem) {
                ...
                this.searchInterval = setTimeout(
                    this.cancelTextSearch.bind(this),
                    1000
                );
            } else {
...
+(function ($) {
    ...
    if ($.oc.table.validator === undefined) $.oc.table.validator = {};
    var Base = function (options) {
        this.options = options;
    };
    Base.prototype.validate = function (value, rowData) {
        if (
            this.options.requiredWith !== undefined &&
            !this.rowHasValue(this.options.requiredWith, rowData)
        )
            return;
    ...
    Base.prototype.getMessage = function (defaultValue) {
        if (this.options.message !== undefined) return this.options.message;
        return defaultValue;
    };
    Base.prototype.rowHasValue = function (columnName, rowData) {
        if (rowData[columnName] === undefined) return false;
        ...
        if (this.options.min !== undefined || this.options.max !== undefined) {
            if (this.options.min !== undefined) {
                if (this.options.min.value === undefined)
                    throw new Error(
                        "The min.value parameter is not defined in the table validator configuration"
                    );
                if (value < this.options.min.value) {
                    return this.options.min.message !== undefined
                        ? this.options.min.message
                        : "The value should not be less than " +
            ...
            if (this.options.max !== undefined) {
                if (this.options.max.value === undefined)
                    throw new Error(
                        "The max.value parameter is not defined in the table validator configuration"
                    );
                if (value > this.options.max.value) {
                    return this.options.max.message !== undefined
                        ? this.options.max.message
                        : "The value should not be more than " +
...
+(function ($) {
    ...
    if ($.oc.table.validator.baseNumber === undefined)
        throw new Error(
            "The $.oc.table.validator.baseNumber namespace is not defined. Make sure that the table.validator.baseNumber.js script is loaded."
        );
        ...
        if (this.options.min !== undefined || this.options.max !== undefined) {
            if (this.options.min !== undefined) {
                ...
                if (value.length < this.options.min.value) {
                    return this.options.min.message !== undefined
                        ? this.options.min.message
                        : "The string should not be shorter than " +
                              this.options.min.value;
                }
            }
            if (this.options.max !== undefined) {
                if (this.options.max.value === undefined)
                    throw new Error(
                        "The max.value parameter is not defined in the Length table validator configuration"
                    );
                if (value.length > this.options.max.value) {
                    return this.options.max.message !== undefined
                        ? this.options.max.message
                        : "The string should not be longer than " +
                              this.options.max.value;
                }
            }
        }
