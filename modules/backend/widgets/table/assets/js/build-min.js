...
+function ($) {
    ...
    Table.prototype.unregisterHandlers=function () {
        this.el.removeEventListener('click',this.clickHandler);document.removeEventListener('click',this.documentClickHandler)
    ...
    Table.prototype.initCellProcessors=function () {
        for (var i=0,len=this.options.columns.length; i<len; i++) {
            var columnConfiguration=this.options.columns[i],column=columnConfiguration.key,columnType=columnConfiguration.type
    ...
    Table.prototype.buildScrollbar=function () {
        var scrollbar=document.createElement('div'),scrollbarContent=document.createElement('div')
    ...
    Table.prototype.buildHeaderTable=function () {
        var headersTable=document.createElement('table'),row=document.createElement('tr')
    ...
    Table.prototype.updateColumnWidth=function () {
        var headerCells=this.headerTable.querySelectorAll('th'),dataCells=this.dataTable.querySelectorAll('tr:first-child td')
    ...
    Table.prototype.buildDataTable=function (records,totalCount) {
        var dataTable=document.createElement('table'),tbody=document.createElement('tbody'),keyColumn=this.options.keyColumn
        ...
        for (var i=0,len=records.length; i<len; i++) {
            ...
            for (var j=0,colsLen=this.options.columns.length; j<colsLen; j++) {
                var cell=document.createElement('td'),dataContainer=document.createElement('input'),cellContentContainer=document.createElement('div'),column=this.options.columns[j],columnName=column.key,cellProcessor=this.getCellProcessor(columnName)
    ...
    Table.prototype.fetchRecords=function (onSuccess) {
        if (this.search.hasQuery()) {
            this.dataSource.searchRecords(this.search.getQuery(),this.navigation.getPageFirstRowOffset(),this.options.recordsPerPage,onSuccess)}
        else {
            this.dataSource.getRecords(this.navigation.getPageFirstRowOffset(),this.options.recordsPerPage,onSuccess)}}
    ...
    Table.prototype.addRecord=function (placement,noFocus) {
        ...
        this.dataSource.createRecord(recordData,placement,relativeToKey,this.navigation.getPageFirstRowOffset(),this.options.recordsPerPage,function onAddRecordDataTableSuccess(records,totalCount)
        {
            self.buildDataTable(records,totalCount)
    ...
    Table.prototype.deleteRecord=function () {
        if (!this.activeCell) {
            ...
            if (paginationEnabled) {
                ...
                this.dataSource.deleteRecord(key,newRecordData,this.navigation.getPageFirstRowOffset(),this.options.recordsPerPage,function onDeleteRecordDataTableSuccess(records,totalCount)
                {
                    self.buildDataTable(records,totalCount)
                    if (!paginationEnabled) {
    ...
    Table.prototype.notifyRowProcessorsOnChange=function (cellElement) {
        var columnName=cellElement.getAttribute('data-column'),row=cellElement.parentNode
    ...
    Table.prototype.validate=function () {
        ...
        for (var i=0,rowsLen=rows.length; i<rowsLen; i++) {
            var row=rows[i],rowData=this.getRowData(row)
            for (var j=0,colsLen=row.children.length; j<colsLen; j++) {
                ...
                for (var columnName in rowData) {
                    var cellProcessor=this.getCellProcessor(columnName),message=cellProcessor.validate(rowData[columnName],rowData)
                    if (message!==undefined) {
                        var cell=row.querySelector('td[data-column="'+columnName+'"]'),self=this
    ...
    Table.prototype.onToolbarClick=function (ev) {
        var target=this.getEventTarget(ev),cmd=target.getAttribute('data-cmd')
        if (!cmd) {
            return}
        switch (cmd) {
            case'record-add':case'record-add-below':this.addRecord('below')
        break
    ...
    Table.prototype.setRowValues=function (rowIndex,rowValues) {
        ...
        for (var i=0,len=row.children.length; i<len; i++) {
            var cell=row.children[i],cellColumnName=this.getCellColumnName(cell)
    ...
    Table.prototype.setCellValue=function (cellElement,value,suppressEvents) {
        ...
        if (dataContainer.value!=value) {
            ...
            if (suppressEvents===undefined||!suppressEvents) {
                this.$el.trigger('oc.tableCellChanged',[this.getCellColumnName(cellElement),value,this.getCellRowIndex(cellElement)])}}}
    ...
    $.fn.table=function (option) {
        ...
        this.each(function () {
            ...
            if (!data) {
                $this.data('oc.table',(data=new Table(this,options)))
    ...
    $(document).on('render',function () {
        $('div[data-control=table]').table()})}(window.jQuery);+function ($) {
            "use strict";if ($.oc.table===undefined) {
            ...
            Navigation.prototype.paginationEnabled=function () {
                return this.tableObj.options.recordsPerPage!=null&&this.tableObj.options.recordsPerPage!=false}
            ...
            Navigation.prototype.buildPaginationLinkList=function (recordCount,recordsPerPage,pageIndex) {
                var pageCount=this.calculatePageCount(recordCount,recordsPerPage),pageList=document.createElement('ul')
            ...
            Navigation.prototype.markActiveLinkItem=function (paginationContainer,pageIndex) {
                var activeItem=paginationContainer.querySelector('.active'),list=paginationContainer.children[0]
            ...
            Navigation.prototype.focusCell=function (rowReference,cellIndex) {
                var row=null,tbody=this.tableObj.getDataTableBody()
            ...
            $.oc.table.helper.navigation=Navigation;}(window.jQuery);+function ($) {
                "use strict";if ($.oc.table===undefined) {
                    throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                }if ($.oc.table.helper===undefined) {
                    $.oc.table.helper={}
                ...
                Search.prototype.onKeydown=function (ev) {
                    ...
                    this.inputTrackTimer=window.setTimeout(function () {
                        self.performSearch(self.searchInput.value)},300)}
                ...
                $.oc.table.helper.search=Search;}(window.jQuery);+function ($) {
                    "use strict";if ($.oc.table===undefined) {
                            throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                    ...
                    Base.prototype.deleteRecord=function (key,newRecordData,offset,count,onSuccess) {
                        onSuccess([],0)}
                    $.oc.table.datasource.base=Base;}(window.jQuery);+function ($) {
                        "use strict";if ($.oc.table===undefined) {
                            throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                        }if ($.oc.table.datasource===undefined) {
                        ...
                        }var Base=$.oc.table.datasource.base,BaseProto=Base.prototype
                        ...
                        Client.prototype.getIndexOfKey=function (key) {
                            ...
                            return this.data.map(function (record) {
                                return record[keyColumn]+""}).indexOf(key+"")}
                        Client.prototype.getAllData=function () {
                            return this.data}
                        $.oc.table.datasource.client=Client}(window.jQuery);+function ($) {
                            "use strict";if ($.oc.table===undefined) {
                                    throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                            }if ($.oc.table.datasource===undefined) {
                                throw new Error("The $.oc.table.datasource namespace is not defined. Make sure that the table.datasource.base.js script is loaded.");
                            ...
                            Server.prototype.getRecords=function (offset,count,onSuccess) {
                                ...
                                this.tableObj.$el.request(handlerName,{data:{offset:offset,count:count}}).done(function (data) {
                            ...
                            Server.prototype.searchRecords=function (query,offset,count,onSuccess) {
                                ...
                                this.tableObj.$el.request(handlerName,{data:{query:query,offset:offset,count:count}}).done(function (data) {
                                    onSuccess(data.records,data.count)})}
                            Server.prototype.createRecord=function (recordData,placement,relativeToKey,offset,count,onSuccess) {
                                ...
                                this.tableObj.$el.request(handlerName,{data:{recordData:recordData,placement:placement,relativeToKey:relativeToKey,offset:offset,count:count}}).done(function (data) {
                                    onSuccess(data.records,data.count)})}
                            ...
                            Server.prototype.deleteRecord=function (key,newRecordData,offset,count,onSuccess) {
                                ...
                                this.tableObj.$el.request(handlerName,{data:{key:key,offset:offset,count:count}}).done(function (data) {
                                    onSuccess(data.records,data.count)})}
                            $.oc.table.datasource.server=Server}(window.jQuery);+function ($) {
                                "use strict";if ($.oc.table===undefined) {
                                    throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                ...
                                $.oc.table.processor.base=Base}(window.jQuery);+function ($) {
                                    "use strict";if ($.oc.table===undefined) {
                                            throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                    }if ($.oc.table.processor===undefined) {
                                    ...
                                    StringProcessor.prototype.setCaretPosition=function (input,position) {
                                        ...
                                        return 0}
                                    $.oc.table.processor.string=StringProcessor;}(window.jQuery);+function ($) {
                                        "use strict";if ($.oc.table===undefined) {
                                            throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                        }if ($.oc.table.processor===undefined) {
                                            throw new Error("The $.oc.table.processor namespace is not defined. Make sure that the table.processor.base.js script is loaded.");
                                        ...
                                        $.oc.table.processor.checkbox=CheckboxProcessor;}(window.jQuery);+function ($) {
                                            "use strict";if ($.oc.table===undefined) {
                                                    throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                            }if ($.oc.table.processor===undefined) {
                                                throw new Error("The $.oc.table.processor namespace is not defined. Make sure that the table.processor.base.js script is loaded.");
                                            ...
                                            DropdownProcessor.prototype.buildEditor=function (cellElement,cellContentContainer,isClick) {
                                                var currentValue=this.tableObj.getCellValue(cellElement),containerPosition=this.getAbsolutePosition(cellContentContainer)
                                                self=this
                                            ...
                                            DropdownProcessor.prototype.fetchOptions=function (cellElement,onSuccess) {
                                                if (this.columnConfiguration.options) {
                                                    ...
                                                    if (!this.cachedOptionPromises[cachingKey]) {
                                                        var requestData={column:this.columnName,rowData:this.tableObj.getRowData(row)},handlerName=this.tableObj.getAlias()+'::onGetDropdownOptions'
                                                    ...
                                                    {
                                                        onSuccess(data.options)}).always(function onDropDownLoadOptionsAlways()
                                                        {
                                                                viewContainer.setAttribute('class','')})}}
                                            DropdownProcessor.prototype.createOptionsCachingKey=function (row) {
                                                var cachingKey='non-dependent',dependsOn=this.columnConfiguration.dependsOn
                                                if (dependsOn) {
                                            ...
                                            DropdownProcessor.prototype.getAbsolutePosition=function (element) {
                                                var top=document.body.scrollTop,left=0
                                                do {
                                                    top+=element.offsetTop||0;top-=element.scrollTop||0;left+=element.offsetLeft||0;element=element.offsetParent;}while (element) {
                                                    returntop:top,left:left}}
                                            ...
                                            DropdownProcessor.prototype.setSelectedItem=function (item) {
                                                if (!this.itemListElement) {
                                                    ...
                                                    this.itemListElement.querySelectorAll('ul li').forEach(function (option) {
                                                        option.removeAttribute('class');});item.setAttribute('class','selected');}
                                            ...
                                            DropdownProcessor.prototype.onItemClick=function (ev) {
                                                ...
                                                if (target.tagName=='LI') {
                                                    target.focus();this.updateCellFromFocusedItem(target)
                                                    this.hideDropdown()}}
                                            ...
                                            DropdownProcessor.prototype.onKeyDown=function (ev) {
                                                if (!this.itemListElement)
                                                return
                                                if ((ev.key==='(Space character)'||ev.key==='Spacebar'||ev.key===' ')&&!this.searching) {
                                                    this.showDropdown()}else if (ev.key==='ArrowDown'||ev.key==='ArrowUp') {
                                                                        var selected=this.findSelectedItem(),newSelectedItem;if (!selected) {
                                                        if (ev.key==='ArrowUp') {
                                                            return false}
                                                        newSelectedItem=this.itemListElement.querySelector('ul li:first-child')}else {
                                            ...
                                            DropdownProcessor.prototype.onRowValueChanged=function (columnName,cellElement) {
                                                ...
                                                if (!dependsOnColumn) {
                                                    ...
                                                    {
                                                        var value=options[currentValue]!==undefined?options[currentValue]:'...'
                                            ...
                                            DropdownProcessor.prototype.searchByTextInput=function (ev,focusOnly) {
                                                if (focusOnly===undefined) {
                                                        focusOnly=false;}
                                                var character=ev.key;if (character.length===1||character==='Space') {
                                                    if (!this.searching) {
                                                            this.searching=true;this.searchQuery='';}
                                                    this.searchQuery+=(character==='Space')?' ':character;var validItem=null;var query=this.searchQuery;this.itemListElement.querySelectorAll('ul li').forEach(function (item) {
                                                        if (validItem===null&&item.dataset.value&&item.dataset.value.toLowerCase().indexOf(query.toLowerCase())===0) {
                                                            validItem=item;}});if (validItem) {
                                                        if (focusOnly===true) {
                                                            validItem.focus();}else {
                                                            this.setSelectedItem(validItem);}
                                                            if (this.searchInterval) {
                                                                clearTimeout(this.searchInterval);}
                                                            this.searchInterval=setTimeout(this.cancelTextSearch.bind(this),1000);}else {
                                                                this.cancelTextSearch();}}}
                                            DropdownProcessor.prototype.cancelTextSearch=function () {
                                                this.searching=false;this.searchQuery=null;this.searchInterval=null;}
                                            $.oc.table.processor.dropdown=DropdownProcessor;}(window.jQuery);+function ($) {
                                                "use strict";if ($.oc.table===undefined) {
                                                    throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                                }if ($.oc.table.processor===undefined) {
                                                    throw new Error("The $.oc.table.processor namespace is not defined. Make sure that the table.processor.base.js script is loaded.");
                                                }var Base=$.oc.table.processor.string,BaseProto=Base.prototype
                                                ...
                                                AutocompleteProcessor.prototype.fetchOptions=function (cellElement,onSuccess) {
                                                    if (this.columnConfiguration.options) {
                                                        if (onSuccess!==undefined) {
                                                            onSuccess(this.columnConfiguration.options)}}else {
                                                        if (this.triggerGetOptions(onSuccess)===false) {
                                                            return}
                                                                var row=cellElement.parentNode,cachingKey=this.createOptionsCachingKey(row),viewContainer=this.getViewContainer(cellElement)
                                                                $.oc.foundation.element.addClass(viewContainer,'loading')
                                                        if (!this.cachedOptionPromises[cachingKey]) {
                                                            var requestData={column:this.columnName,rowData:this.tableObj.getRowData(row)},handlerName=this.tableObj.getAlias()+'::onGetAutocompleteOptions'
                                                            this.cachedOptionPromises[cachingKey]=this.tableObj.$el.request(handlerName,{data:requestData})}
                                                        this.cachedOptionPromises[cachingKey].done(function onAutocompleteLoadOptionsSuccess(data)
                                                        {
                                                            if (onSuccess!==undefined) {
                                                                onSuccess(data.options)}}).always(function onAutocompleteLoadOptionsAlways()
                                                                {
                                                ...
                                                $.oc.table.processor.autocomplete=AutocompleteProcessor;}(window.jQuery);+function ($) {
                                                    "use strict";if ($.oc.table===undefined) {
                                                            throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                                    ...
                                                    $.oc.table.validator.base=Base;}(window.jQuery);+function ($) {
                                                        "use strict";if ($.oc.table===undefined) {
                                                            throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                                        }if ($.oc.table.validator===undefined) {
                                                        ...
                                                        }var Base=$.oc.table.validator.base,BaseProto=Base.prototype
                                                        var Required=function (options) {
                                                                    Base.call(this,options)};Required.prototype=Object.create(BaseProto)
                                                        ...
                                                        $.oc.table.validator.required=Required}(window.jQuery);+function ($) {
                                                            "use strict";if ($.oc.table===undefined) {
                                                                    throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                                            }if ($.oc.table.validator===undefined) {
                                                                throw new Error("The $.oc.table.validator namespace is not defined. Make sure that the table.validator.base.js script is loaded.");
                                                            ...
                                                            var BaseNumber=function (options) {
                                                                                Base.call(this,options)};BaseNumber.prototype=Object.create(BaseProto)
                                                                    ...
                                                                    if (this.options.min!==undefined) {
                                                                        if (this.options.min.value===undefined) {
                                                                            ...
                                                                            if (value<this.options.min.value) {
                                                                                return this.options.min.message!==undefined?this.options.min.message:"The value should not be less than "+this.options.min.value}
                                                                        }}
                                                                    if (this.options.max!==undefined) {
                                                                        if (this.options.max.value===undefined) {
                                                                            ...
                                                                            if (value>this.options.max.value) {
                                                                                return this.options.max.message!==undefined?this.options.max.message:"The value should not be more than "+this.options.max.value}
                                                                        }}}
                                                                return}
                                                            $.oc.table.validator.baseNumber=BaseNumber}(window.jQuery);+function ($) {
                                                                "use strict";if ($.oc.table===undefined) {
                                                                    throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                                                }if ($.oc.table.validator===undefined) {
                                                                    throw new Error("The $.oc.table.validator namespace is not defined. Make sure that the table.validator.base.js script is loaded.");
                                                                ...
                                                                }var Base=$.oc.table.validator.baseNumber,BaseProto=Base.prototype
                                                                var Integer=function (options) {
                                                                                Base.call(this,options)};Integer.prototype=Object.create(BaseProto)
                                                                ...
                                                                Integer.prototype.validateValue=function (value,rowData) {
                                                                    ...
                                                                    if (value.length==0) {
                                                                        ...
                                                                        if (!testResult) {
                                                                                var defaultMessage=this.options.allowNegative?'The value should be an integer.':'The value should be a positive integer';return this.getMessage(defaultMessage)}
                                                                    }
                                                                    return this.doCommonChecks(parseInt(value))}
                                                                $.oc.table.validator.integer=Integer}(window.jQuery);+function ($) {
                                                                    "use strict";if ($.oc.table===undefined) {
                                                                            throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                                                    }if ($.oc.table.validator===undefined) {
                                                                        throw new Error("The $.oc.table.validator namespace is not defined. Make sure that the table.validator.base.js script is loaded.");
                                                                    ...
                                                                    var Float=function (options) {
                                                                                Base.call(this,options)};Float.prototype=Object.create(BaseProto)
                                                                    ...
                                                                    Float.prototype.validateValue=function (value,rowData) {
                                                                        ...
                                                                        if (value.length==0) {
                                                                            ...
                                                                            if (!testResult) {
                                                                                var defaultMessage=this.options.allowNegative?'The value should be a floating point number.':'The value should be a positive floating point number';return this.getMessage(defaultMessage)}
                                                                        }
                                                                        return this.doCommonChecks(parseFloat(value))}
                                                                    $.oc.table.validator.float=Float}(window.jQuery);+function ($) {
                                                                        "use strict";if ($.oc.table===undefined) {
                                                                            throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                                                        }if ($.oc.table.validator===undefined) {
                                                                            throw new Error("The $.oc.table.validator namespace is not defined. Make sure that the table.validator.base.js script is loaded.");
                                                                        ...
                                                                        var Length=function (options) {
                                                                                            Base.call(this,options)};Length.prototype=Object.create(BaseProto)
                                                                                    ...
                                                                                    if (this.options.min!==undefined) {
                                                                                        if (this.options.min.value===undefined) {
                                                                                            ...
                                                                                            if (value.length<this.options.min.value) {
                                                                                                return this.options.min.message!==undefined?this.options.min.message:"The string should not be shorter than "+this.options.min.value}
                                                                                        }}
                                                                                    if (this.options.max!==undefined) {
                                                                                        if (this.options.max.value===undefined) {
                                                                                            ...
                                                                                            if (value.length>this.options.max.value) {
                                                                                                return this.options.max.message!==undefined?this.options.max.message:"The string should not be longer than "+this.options.max.value}
                                                                                        }}}
                                                                            }
                                                                            return}
                                                                        $.oc.table.validator.length=Length}(window.jQuery);+function ($) {
                                                                            "use strict";if ($.oc.table===undefined) {
                                                                                    throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");
                                                                            }if ($.oc.table.validator===undefined) {
                                                                                throw new Error("The $.oc.table.validator namespace is not defined. Make sure that the table.validator.base.js script is loaded.");
                                                                            ...
                                                                            var Regex=function (options) {
                                                                                            Base.call(this,options)};Regex.prototype=Object.create(BaseProto)
