+(function ($) {
    ...
    MediaManager.prototype.getSelectedItems = function (
        ...
        var items = this.$el
                .get(0)
                .querySelectorAll('[data-type="media-item"].selected'),
            result = [];
        if (!allowRootItem) {
            var filteredItems = [];
            for (var i = 0, len = items.length; i < len; i++) {
                var item = items[i];
                if (!item.hasAttribute("data-root")) filteredItems.push(item);
            }
            items = filteredItems;
        }
        if (returnNotProcessed === true) return items;
        for (var i = 0, len = items.length; i < len; i++) {
            var item = items[i],
                itemDetails = {
    ...
    MediaManager.prototype.init = function () {
        this.itemListElement = this.$el
            .find('[data-control="item-list"]')
            .get(0);
        this.scrollContentElement = this.itemListElement.querySelector(
            ".scroll-wrapper"
        );
        if (this.options.bottomToolbar) {
            this.$el
                .find('[data-control="bottom-toolbar"]')
                .removeClass("hide");
            if (this.options.cropAndInsertButton)
                this.$el
                    .find('[data-popup-command="crop-and-insert"]')
                    .removeClass("hide");
        }
    ...
    MediaManager.prototype.registerHandlers = function () {
        ...
        this.$el.on(
            "click.tree-path",
            'ul.tree-path, [data-control="sidebar-labels"]',
            this.proxy(this.onNavigate)
        );
        this.$el.on(
            "click.command",
            "[data-command]",
            this.proxy(this.onCommandClick)
        );
        this.$el.on(
            "click.item",
            '[data-type="media-item"]',
            this.proxy(this.onItemClick)
        );
        this.$el.on(
            "touchend",
            '[data-type="media-item"]',
            this.proxy(this.onItemTouch)
        );
        this.$el.on(
            "change",
            '[data-control="sorting"]',
            this.proxy(this.onSortingChanged)
        );
        this.$el.on(
            "input",
            '[data-control="search"]',
            this.proxy(this.onSearchChanged)
        );
        this.$el.on("mediarefresh", this.proxy(this.refresh));
        this.$el.on(
            "shown.oc.popup",
            '[data-command="create-folder"]',
            this.proxy(this.onFolderPopupShown)
        );
        this.$el.on(
            "hidden.oc.popup",
            '[data-command="create-folder"]',
            this.proxy(this.onFolderPopupHidden)
        );
        this.$el.on(
            "shown.oc.popup",
            '[data-command="move"]',
            this.proxy(this.onMovePopupShown)
        );
        this.$el.on(
            "hidden.oc.popup",
            '[data-command="move"]',
            this.proxy(this.onMovePopupHidden)
        );
        this.$el.on("keydown", this.proxy(this.onKeyDown));
        if (this.itemListElement)
            this.itemListElement.addEventListener(
                "mousedown",
                this.proxy(this.onListMouseDown)
            );
    };
    MediaManager.prototype.unregisterHandlers = function () {
        ...
        if (this.itemListElement) {
            this.itemListElement.removeEventListener(
                "mousedown",
                this.proxy(this.onListMouseDown)
            );
            this.itemListElement.removeEventListener(
                "mousemove",
                this.proxy(this.onListMouseMove)
            );
        }
    ...
    MediaManager.prototype.clearSelectTimer = function () {
        if (this.selectTimer === null) return;
    ...
    MediaManager.prototype.selectItem = function (node, expandSelection) {
        if (!expandSelection) {
            ...
            else node.setAttribute("class", "selected");
        }
        node.focus();
        this.clearSelectTimer();
        if (this.isPreviewSidebarVisible()) {
            this.selectTimer = setTimeout(
                this.proxy(this.updateSidebarPreview),
                100
            );
        }
    ...
    MediaManager.prototype.unselectRoot = function () {
        var rootItem = this.$el
            .get(0)
            .querySelector('[data-type="media-item"][data-root].selected');
        if (rootItem) rootItem.setAttribute("class", "");
    };
    MediaManager.prototype.clearDblTouchTimer = function () {
        if (this.dblTouchTimer === null) return;
    ...
    MediaManager.prototype.selectFirstItem = function () {
        var firstItem = this.itemListElement.querySelector(
            '[data-type="media-item"]:first-child'
        );
        if (firstItem) {
    ...
    MediaManager.prototype.selectRelative = function (next, expandSelection) {
        ...
        if (next) {
            ...
            if (lastItem) itemToSelect = lastItem.nextElementSibling;
        } else {
            var firstItem = currentSelection[0];
            if (firstItem) itemToSelect = firstItem.previousElementSibling;
        }
        if (itemToSelect) this.selectItem(itemToSelect, expandSelection);
    };
    ...
    MediaManager.prototype.execNavigationRequest = function (
        handler,
        data,
        element
    ) {
        if (element === undefined) element = this.$form;
        ...
        this.navigationAjax = element
            .request(this.options.alias + "::" + handler, { data: data })
            .always(function () {
    ...
    MediaManager.prototype.navigateToItem = function ($item) {
        if (!$item.length || !$item.data("path").length) return;
    ...
    MediaManager.prototype.isPreviewSidebarVisible = function () {
        return !this.$el
            .find('[data-control="preview-sidebar"]')
            .hasClass("hide");
    };
    ...
    MediaManager.prototype.updateSidebarMediaPreview = function (items) {
        ...
        if (items.length == 1 && !items[0].hasAttribute("data-root")) {
            ...
            switch (documentType) {
                case "audio":
                    template = previewPanel.querySelector(
                        '[data-control="audio-template"]'
                    ).innerHTML;
                    break;
                case "video":
                    template = previewPanel.querySelector(
                        '[data-control="video-template"]'
                    ).innerHTML;
                    break;
                case "image":
                    template = previewPanel.querySelector(
                        '[data-control="image-template"]'
                    ).innerHTML;
                    break;
            }
            ...
            if (documentType == "image") this.loadSidebarThumbnail();
        } else if (items.length == 1 && items[0].hasAttribute("data-root")) {
            template = previewPanel.querySelector('[data-control="go-up"]')
                .innerHTML;
            previewContainer.innerHTML = template;
        } else if (items.length == 0) {
            template = previewPanel.querySelector(
                '[data-control="no-selection-template"]'
            ).innerHTML;
            previewContainer.innerHTML = template;
        } else {
            template = previewPanel.querySelector(
                '[data-control="multi-selection-template"]'
            ).innerHTML;
            previewContainer.innerHTML = template;
        }
    };
    MediaManager.prototype.updateSidebarPreview = function (resetSidebar) {
        if (!this.sidebarPreviewElement)
            this.sidebarPreviewElement = this.$el
                .get(0)
                .querySelector('[data-control="preview-sidebar"]');
        var items =
                resetSidebar === undefined
                    ? this.$el
                          .get(0)
                          .querySelectorAll('[data-type="media-item"].selected')
                    : [],
        ...
        if (items.length == 0) {
            this.sidebarPreviewElement
                .querySelector('[data-control="sidebar-labels"]')
            ...
            previewPanel.querySelector(
                '[data-label="size"]'
            ).textContent = item.getAttribute("data-size");
            previewPanel.querySelector(
                '[data-label="title"]'
            ).textContent = item.getAttribute("data-title");
            previewPanel.querySelector(
                '[data-label="last-modified"]'
            ).textContent = lastModified;
            previewPanel
                .querySelector('[data-label="public-url"]')
                .setAttribute("href", item.getAttribute("data-public-url"));
            if (lastModified)
                previewPanel
                    .querySelector('[data-control="last-modified"]')
                    .setAttribute("class", "");
            else
                previewPanel
                    .querySelector('[data-control="last-modified"]')
            ...
            if (this.isSearchMode()) {
                previewPanel
                    .querySelector('[data-control="item-folder"]')
                    .setAttribute("class", "");
                var folderNode = previewPanel.querySelector(
                    '[data-label="folder"]'
                );
                folderNode.textContent = item.getAttribute("data-folder");
                folderNode.setAttribute(
                    "data-path",
                    item.getAttribute("data-folder")
                );
            } else {
                previewPanel
                    .querySelector('[data-control="item-folder"]')
    ...
    MediaManager.prototype.loadSidebarThumbnail = function () {
        ...
        var sidebarThumbnail = this.sidebarPreviewElement.querySelector(
            '[data-control="sidebar-thumbnail"]'
        );
        ...
        this.sidebarThumbnailAjax = this.$form
            .request(this.options.alias + "::onGetSidebarThumbnail", {
                data: data,
            })
    ...
    MediaManager.prototype.replaceSidebarPlaceholder = function (response) {
        if (!this.sidebarPreviewElement) return;
        var sidebarThumbnail = this.sidebarPreviewElement.querySelector(
            '[data-control="sidebar-thumbnail"]'
        );
        if (!sidebarThumbnail) return;
    ...
    MediaManager.prototype.generateThumbnails = function () {
        this.thumbnailQueue = [];
        var placeholders = this.itemListElement.querySelectorAll(
            '[data-type="media-item"] div.image-placeholder'
        );
        for (var i = placeholders.length - 1; i >= 0; i--)
            this.thumbnailQueue.push({
                ...
                lastModified: placeholders[i].getAttribute(
                    "data-last-modified"
                ),
            });
    ...
    MediaManager.prototype.handleThumbnailQueue = function () {
        ...
        for (
            var i = this.activeThumbnailQueueLength;
            i < maxThumbnailQueueLength && this.thumbnailQueue.length > 0;
            i++
        ) {
            var batch = [];
            for (
                var j = 0;
                j < maxThumbnailBatchLength && this.thumbnailQueue.length > 0;
                j++
            )
                batch.push(this.thumbnailQueue.pop());
            this.activeThumbnailQueueLength++;
            this.handleThumbnailBatch(batch).always(
                this.proxy(this.placeholdersUpdated)
            );
        }
    };
    MediaManager.prototype.handleThumbnailBatch = function (batch) {
        ...
        for (var i = 0, len = batch.length; i < len; i++) {
            ...
            if (placeholder) placeholder.setAttribute("data-loading", "true");
        }
        var promise = this.$form.request(
            this.options.alias + "::onGenerateThumbnails",
            { data: data }
        );
    ...
    MediaManager.prototype.replacePlaceholder = function (response) {
        if (!response.generatedThumbnails) return;
        for (
            var i = 0, len = response.generatedThumbnails.length;
            i < len;
            i++
        ) {
    ...
    MediaManager.prototype.getRelativePosition = function (
        element,
        pageX,
        pageY,
        startPosition
    ...
    MediaManager.prototype.createSelectionMarker = function () {
        ...
        this.scrollContentElement.insertBefore(
            this.selectionMarker,
            this.scrollContentElement.firstChild
        );
    };
    MediaManager.prototype.doObjectsCollide = function (
        aTop,
        aLeft,
        aWidth,
        aHeight,
    ...
    MediaManager.prototype.initUploader = function () {
        if (!this.itemListElement || this.options.readOnly) return;
        var uploaderOptions = {
        ...
        this.dropzone.on(
            "totaluploadprogress",
            this.proxy(this.uploadUpdateTotalProgress)
        );
    ...
    MediaManager.prototype.uploadUpdateTotalProgress = function (
        uploadProgress,
        totalBytes,
        totalBytesSent
    ) {
        this.setUploadProgress(uploadProgress);
        var fileNumberLabel = this.$el
                .get(0)
                .querySelector('[data-label="file-number-and-progress"]'),
            messageTemplate = fileNumberLabel.getAttribute(
        ...
        fileNumberLabel.innerHTML = messageTemplate
            .replace(":number", fileNumber)
            .replace(":percents", Math.round(uploadProgress) + "%");
    };
    MediaManager.prototype.setUploadProgress = function (value) {
        var progressBar = this.$el
            .get(0)
            .querySelector('[data-control="upload-progress-bar"]');
    ...
    MediaManager.prototype.updateUploadBar = function (
        templateName,
        classNames
    ) {
        var fileNumberLabel = this.$el
                .get(0)
                .querySelector('[data-label="file-number-and-progress"]'),
            successTemplate = fileNumberLabel.getAttribute(
    ...
    MediaManager.prototype.clearSearchTrackInputTimer = function () {
        if (this.searchTrackInputTimer === null) return;
    ...
    MediaManager.prototype.onSearchChanged = function (ev) {
        ...
        this.clearSearchTrackInputTimer();
        this.searchTrackInputTimer = window.setTimeout(
            this.proxy(this.updateSearchResults),
            300
        );
    };
    ...
    MediaManager.prototype.deleteConfirmation = function (confirmed) {
        ...
        $.oc.stripeLoadIndicator.show();
        this.$form
            .request(this.options.alias + "::onDeleteItem", { data: data })
            .always(function () {
                $.oc.stripeLoadIndicator.hide();
            })
            .done(this.proxy(this.afterNavigate));
    ...
    MediaManager.prototype.createFolder = function (ev) {
        $(ev.target).popup({
            content: this.$el
                .find('[data-control="new-folder-template"]')
                .html(),
            zIndex: 1200,
        });
    };
    ...
    MediaManager.prototype.onNewFolderSubmit = function (ev) {
        ...
        $.oc.stripeLoadIndicator.show();
        this.$form
            .request(this.options.alias + "::onCreateFolder", { data: data })
            .always(function () {
                $.oc.stripeLoadIndicator.hide();
            })
    ...
    MediaManager.prototype.onMoveItemsSubmit = function (ev) {
        var items = this.$el
            ...
            data = {
                ...
                originalPath: $(ev.target)
                    .find("input[name=originalPath]")
                    .val(),
                files: [],
                folders: [],
            };
        for (var i = 0, len = items.length; i < len; i++) {
            ...
            else data.files.push(path);
        }
        $.oc.stripeLoadIndicator.show();
        this.$form
            .request(this.options.alias + "::onMoveItems", { data: data })
            .always(function () {
                $.oc.stripeLoadIndicator.hide();
            })
    ...
    MediaManager.prototype.onItemClick = function (ev) {
        if (
            ev.target.tagName == "I" &&
            ev.target.hasAttribute("data-rename-control")
        )
            return;
    ...
    MediaManager.prototype.onItemTouch = function (ev) {
        ...
        this.clearDblTouchTimer();
        this.dblTouchTimer = setTimeout(
            this.proxy(this.clearDblTouchFlag),
            300
        );
    };
    MediaManager.prototype.onListMouseDown = function (ev) {
        ...
        this.itemListPosition = $.oc.foundation.element.absolutePosition(
    ...
    MediaManager.prototype.onListMouseUp = function (ev) {
        ...
        if (this.selectionStarted) {
            this.unselectRoot();
            var items = this.itemListElement.querySelectorAll(
                    '[data-type="media-item"]:not([data-root])'
                ),
                selectionPosition = $.oc.foundation.element.absolutePosition(
                    this.selectionMarker,
                    true
                );
            for (var index = 0, len = items.length; index < len; index++) {
                var item = items[index],
                    itemPosition = $.oc.foundation.element.absolutePosition(
                        item,
                        true
                    );
                if (
                    this.doObjectsCollide(
                    ...
                    if (!ev.shiftKey) item.setAttribute("class", "selected");
                    else {
                        if (item.getAttribute("class") == "selected")
                            item.setAttribute("class", "");
                        else item.setAttribute("class", "selected");
                    }
                } else if (!ev.shiftKey) item.setAttribute("class", "");
            }
            this.updateSidebarPreview();
    ...
    MediaManager.prototype.onListMouseMove = function (ev) {
        ...
        if (this.selectionStarted) {
            if (deltaX >= 0) {
                this.selectionMarker.style.left =
                    this.selectionStartPoint.x + "px";
            ...
            if (deltaY >= 0) {
                ...
                this.selectionMarker.style.top =
                    this.selectionStartPoint.y + "px";
            } else {
    ...
    $.fn.mediaManager = function (option) {
        ...
        this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.mediaManager",
                    (data = new MediaManager(this, options))
                );
...
+(function ($) {
    ...
    MediaManagerImageCropPopup.prototype.unregisterHandlers = function () {
        this.$popupElement.off(
            "change",
            '[data-control="selection-mode"]',
            this.proxy(this.onSelectionModeChanged)
        );
        this.$popupElement.off(
            "click",
            "[data-command]",
        ...
        this.$popupElement.off(
            "shown.oc.popup",
            "button[data-command=resize]",
            this.proxy(this.onResizePopupShown)
        );
        this.$popupElement.off(
            "hidden.oc.popup",
            "button[data-command=resize]",
            this.proxy(this.onResizePopupHidden)
        );
        if (this.rulersVisible) {
            var $cropToolRoot = this.$popupElement.find(
                "[data-control=media-manager-crop-tool]"
            );
            this.imageArea.removeEventListener(
                "scroll",
                this.proxy(this.onImageScroll)
            );
        }
    ...
    MediaManagerImageCropPopup.prototype.removeAttachedControls = function () {
        if (this.$popupElement) {
            this.$popupElement
                .find('[data-control="selection-mode"]')
                .select2("destroy")
                .remove();
            this.$popupElement
                .find("[data-control=toolbar]")
                .toolbar("dispose")
                .remove();
            this.jCrop.destroy();
        }
    ...
    MediaManagerImageCropPopup.prototype.hide = function () {
        if (this.$popupElement) this.$popupElement.trigger("close.oc.popup");
    };
    ...
    MediaManagerImageCropPopup.prototype.initRulers = function () {
        ...
        this.hRulerHolder = $cropToolRoot
            .find(".ruler-container.horizontal .layout-relative")
            .get(0);
        this.vRulerHolder = $cropToolRoot
            .find(".ruler-container.vertical .layout-relative")
            .get(0);
    };
    MediaManagerImageCropPopup.prototype.createRulerTicks = function (
        $rulerElement,
        count
    ) {
    ...
    MediaManagerImageCropPopup.prototype.cropAndInsert = function () {
        ...
        this.$popupElement
            .find("form")
            .request(this.options.alias + "::onCropImage", { data: data })
            .always(function () {
                $.oc.stripeLoadIndicator.hide();
            })
    ...
    MediaManagerImageCropPopup.prototype.onResizePopupShown = function (
        ...
        var $popup = $(popup),
            ...
            imageWidth = this.fixDimensionValue(
                this.$popupElement
                    .find("input[data-control=dimension-width]")
                    .val()
            ),
            imageHeight = this.fixDimensionValue(
                this.$popupElement
                    .find("input[data-control=dimension-height]")
                    .val()
            );
        ...
        $widthControl.on(
            "keyup.media",
            this.proxy(this.onResizeDimensionKeyUp)
        );
        $heightControl.on(
            "keyup.media",
            this.proxy(this.onResizeDimensionKeyUp)
        ...
        $widthControl.on(
            "change.media",
            this.proxy(this.onResizeDimensionChanged)
        );
        $heightControl.on(
            "change.media",
            this.proxy(this.onResizeDimensionChanged)
    ...
    MediaManagerImageCropPopup.prototype.onResizePopupHidden = function (
        ev,
        button,
        popup
    ) {
    ...
    MediaManagerImageCropPopup.prototype.onResizeDimensionKeyUp = function (
        ev
    ) {
        var $target = $(ev.target),
            targetValue = this.fixDimensionValue($target.val()),
            otherDimensionName =
                $target.attr("name") == "width" ? "height" : "width",
            $otherInput = $target
                .closest("form")
                .find("input[name=" + otherDimensionName + "]"),
            ratio = this.$popupElement
                .find("[data-control=original-ratio]")
                .val(),
            value =
    ...
    MediaManagerImageCropPopup.prototype.onResizeDimensionChanged = function (
        ev
    ) {
        var $target = $(ev.target);
    ...
    MediaManagerImageCropPopup.prototype.onResizeSubmit = function (ev) {
        var data = {
            cropSessionKey: this.$popupElement
                .find("input[name=cropSessionKey]")
                .val(),
            path: this.$popupElement.find("input[name=path]").val(),
        };
        $.oc.stripeLoadIndicator.show();
        $(ev.target)
            .request(this.options.alias + "::onResizeImage", { data: data })
            .always(function () {
                $.oc.stripeLoadIndicator.hide();
            })
    ...
    MediaManagerImageCropPopup.prototype.imageResized = function (response) {
        ...
        this.updateImage(
            ...
            response.dimensions[1]
        );
    };
    MediaManagerImageCropPopup.prototype.updateImage = function (
        url,
        width,
        hegiht
    ) {
    ...
    MediaManagerImageCropPopup.prototype.updateSelectionSizeLabel = function (
        width,
        height
    ) {
        if (width == 0 && height == 0) {
        ...
        this.selectionSizeLabel.querySelector(
            "[data-label=selection-width]"
        ).textContent = parseInt(width);
        this.selectionSizeLabel.querySelector(
            "[data-label=selection-height]"
        ).textContent = parseInt(height);
    };
    MediaManagerImageCropPopup.prototype.onPopupHidden = function (
        event,
        element,
        popup
    ) {
        this.$popupElement
            .find("form")
            .request(this.options.alias + "::onEndCroppingSession");
        $(document).trigger("mousedown");
        this.dispose();
    };
    MediaManagerImageCropPopup.prototype.onPopupShown = function (
        event,
        element,
        popup
    ) {
        ...
        this.imageArea = popup
            .find("[data-control=media-manager-crop-tool]")
            .get(0)
            .querySelector(".image_area");
        this.selectionSizeLabel = popup
            .find('[data-label="selection-size"]')
            .get(0);
