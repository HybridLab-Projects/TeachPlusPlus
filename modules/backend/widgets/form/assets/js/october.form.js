...
+(function ($) {
    ...
    FormWidget.prototype.init = function () {
        ...
        this.$el.on(
            "oc.triggerOn.afterUpdate",
            this.proxy(this.toggleEmptyTabs)
        );
    ...
    FormWidget.prototype.getFieldElements = function () {
        ...
        var form = this.$el,
            nestedFields = form.find(
                '[data-control="formwidget"] [data-field-name]'
            );

        return (this.fieldElementCache = form
    ...
    FormWidget.prototype.onRefreshDependants = function (fieldName, toRefresh) {
        ...
        this.dependantUpdateTimers[fieldName] = window.setTimeout(function () {
            ...
            formEl
                .request(self.options.refreshHandler, {
                    data: refreshData,
                })
                .success(function () {
        ...
        $.each(toRefresh.fields, function (index, field) {
            fieldElements
                .filter('[data-field-name="' + field + '"]:visible')
    ...
    FormWidget.prototype.bindLazyTabs = function () {
        ...
        if ($("> li.active.tab-lazy", tabContainer).length) {
            $(
                '> li.active.tab-lazy > [data-toggle="tab"]',
                tabContainer
            ).trigger("click");
        }
    };
    ...
    FormWidget.prototype.toggleEmptyTabs = function () {
        ...
        this.toggleEmptyTabsTimer = window.setTimeout(function () {
            ...
            if (
                !tabControl.length ||
                !$.contains(form.get(0), tabControl.get(0))
            )
                return;
            ...
            $(".tab-pane:not(.lazy)", tabControl).each(function () {
                $('[data-target="#' + $(this).attr("id") + '"]', tabControl)
                    .closest("li")
                    .toggle(
                        !!$("> .form-group:not(:empty):not(.hide)", $(this))
                            .length
                    );
            });
    ...
    $.fn.formWidget = function (option) {
        ...
        this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.formwidget",
                    (data = new FormWidget(this, options))
                );
