...
+(function ($) {
    ...
    ReportContainer.prototype.init = function () {
            ...
            self.$form.request(self.alias + "::onUpdateWidget", {
                ...
                success: function (data) {
                    this.success(data).done(function () {
                        ...
                        $(li).addClass(
                            "width-" + parsedValues["ocWidgetWidth"]
                        );
                        $(li).toggleClass(
                            "new-line",
                            parsedValues["ocWidgetNewRow"] == 1
                        );
        ...
        this.$el.on("click", ".content > button.close", function () {
            var $btn = $(this);
            $.oc.confirm(
                $.oc.lang.get("alert.widget_remove_confirm"),
                function () {
                    self.$form.request(self.alias + "::onRemoveWidget", {
                        data: {
                            alias: $(
                                "[data-widget-alias]",
                                $btn.closest("div.content")
                            ).val(),
                        },
                    });
    ...
    $.fn.reportContainer = function (option) {
        return this.each(function () {
            ...
            if (!data)
                $this.data(
                    "oc.reportContainer",
                    (data = new ReportContainer(this, options))
                );
